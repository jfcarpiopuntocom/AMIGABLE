...
(function () {
  ...
  const OCSync = (function () {
    ...
    function ultimoAplicado(dev) {
      try { return (JSON.parse(localStorage.getItem("oc_sync_last") || "{}"))[dev] || 0; } catch { return 0; }
    }
    function marcarAplicado(dev, ts) {
      let mapa = {}; try { mapa = JSON.parse(localStorage.getItem("oc_sync_last") || "{}"); } catch (_) {}
      if (ts > (mapa[dev] || 0)) { mapa[dev] = ts; localStorage.setItem("oc_sync_last", JSON.stringify(mapa)); }
    }

    // Reproduce las operaciones de OTROS dispositivos contra el backend
    // local, en orden cronológico, saltando lo ya aplicado antes.
    async function reproducir(ops) {
      const porDispositivo = {};
      ops.forEach((op) => { if (op.dev !== deviceId()) (porDispositivo[op.dev] = porDispositivo[op.dev] || []).push(op); });
      for (const dev in porDispositivo) {
        const pendientes = porDispositivo[dev].filter((op) => op.ts > ultimoAplicado(dev)).sort((a, b) => a.ts - b.ts);
        let maxTs = ultimoAplicado(dev);
        for (const op of pendientes) {
          try {
            await fetchOriginal(op.url, { method: op.method, headers: { "Content-Type": "application/json" }, body: op.body });
            maxTs = Math.max(maxTs, op.ts);
          } catch (_) { /* si una falla, seguimos con las demás */ }
        }
        marcarAplicado(dev, maxTs);
      }
    }
    ...
    async function push() {
      ...
      try {
        ...
        const enviado = cola.length;
        cola = [];
        localStorage.removeItem("oc_sync_pending");
    ...
    function arrancarIntervalo() {
      if (temporizador) clearInterval(temporizador);
      temporizador = setInterval(() => { push().then(pull); }, 4 * 60 * 1000);
      window.addEventListener("online", () => push().then(pull));
    }
    ...
    async function generarPaqueteManual() {
      ...
      const texto = "OCSYNC1:" + btoa(unescape(encodeURIComponent(JSON.stringify(paquete))));
      cola = [];
      localStorage.removeItem("oc_sync_pending");
      return texto;
    }
