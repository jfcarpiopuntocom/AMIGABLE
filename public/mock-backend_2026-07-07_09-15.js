...
(function () {
  ...
  const OC_STATE_KEY = "amigable_demo_state_v2";
  ...
  function estadoActualExportable() {
    return {
      ...
      sucursales: clonar(sucursales), promotoras: clonar(promotoras),
  ...
  function guardarEstadoLocal() { try { localStorage.setItem(OC_STATE_KEY, JSON.stringify(estadoActualExportable())); } catch (_) { /* localStorage lleno: no romper la app */ } }
  ...
  function diasSinVentaDe(p) {
    const ultima = ventas.filter((v) => v.productoId === p.id).reduce((mx, v) => (v.fecha > mx ? v.fecha : mx), "");
  ...
  // Item 3 (revisión JFC 2026-07-05): el QR usaba la API pública qrserver.com,
  // lo que rompía el principio "sin internet". FEATURE — DORMANT: mientras no
  // haya una librería QR local auditada, la etiqueta usa SOLO el código de
  // barras (barcode128.js, 100% local); la UI ya tolera qrDataUrl null. Para
  // re-habilitar QR: generar localmente (ej. librería "qrcode" empaquetada)
  // y devolver aquí su data URL. NO volver a apuntar a APIs externas.
  function qrDataUrl(_payload) { return null; }
  ...
  window.fetch = async function (url, opts) {
    ...
    try {
      ...
      if ((m = path.match(/^\/api\/productos\/([^/]+)\/etiqueta$/))) {
        ...
        // externas. QR: deshabilitado (item 3, ver qrDataUrl arriba) hasta tener
        // generador 100% local; la UI omite el <img> cuando qrDataUrl es null.
