  ...
  function init() {
    ...
    $("oc-exportar").addEventListener("click", async () => {
      try {
        ...
        if (clave === null) { msg("oc-respaldo-msg", "Exportación cancelada.", "var(--ink)"); return; }
        let archivoFinal;
        if (clave && clave.trim()) {
    ...
    $("oc-importar-file").addEventListener("change", async (e) => {
      ...
      try {
        ...
        if ((paquete.schemaVersion || 1) > 2) { msg("oc-respaldo-msg", "Este respaldo es de una versión más nueva de AMIGABLE que esta pantalla — actualiza la app antes de importarlo.", "var(--rojo)"); return; }
    ...
    async function cajaRestaurar(idx) {
      ...
      msg("oc-respaldo-msg", "Restaurado. Recarga la página para ver los datos.", "var(--verde)");
    }
    ...
    cajaPintarAlerta();
    setInterval(() => cajaGuardarPunto(true), CAJA_INTERVALO_MS);
