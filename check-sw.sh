...
sw_ver=$(grep -oE 'amigable-shell-v[0-9]+' docs/sw.js | head -1)
vj_ver=$(grep -oE 'amigable-shell-v[0-9]+' docs/version.json | head -1)
...
ajenas=$(grep -rnE '=\s*"(f123|c123)_[a-z_]+"' docs/*.js 2>/dev/null | grep -vE '_[0-9]{4}-[0-9]{2}-[0-9]{2}_' | grep -vi "VIEJA" | sort -u)
if [ -n "$ajenas" ]; then
  echo "CLAVES DE OTRA APP (G2): esta app es amigable_/amg_, pero aparecen literales ajenos:"
  echo "$ajenas" | sed 's/^/  /'
  falta=1
fi

# G4 (JFC 2026-08-20, plan de guards): todo boton data-vista del <nav> tiene
# que tener su id="vista-<mismo nombre>" correspondiente.
...
if [ -n "$huerfanas_seccion" ]; then
  echo "AVISO (G4): seccion(es) vista-* sin boton de nav que la alcance — revisar a mano:"
...
# manifest esta viejo, el SW verificaria hashes equivocados.
if [ ! -f docs/version-manifest.json ]; then
  echo "FALTA docs/version-manifest.json — corre: node scripts/gen-manifest.js"
  falta=1
else
  man_ver=$(grep -oE '"shell":\s*"amigable-shell-v[0-9]+"' docs/version-manifest.json | grep -oE 'amigable-shell-v[0-9]+' | head -1)
...
# VERIFICACION DE HASHES REALES (JFC 2026-09-08, portada de friendly, mejora #2
# de la auditoria de version). ANTES este script solo comparaba el STRING de
# version entre sw.js, version.json y el manifest -- por eso un cambio a un
# archivo del shell SIN regenerar version-manifest.json pasaba la compuerta y el
# service worker luego lo rechazaba en los aparatos (el incidente con auth-ui.js).
# Ahora se recomputa el SHA-256 REAL de cada archivo del manifest y se compara:
# si uno no cuadra, la compuerta FALLA y pide regenerar.
if [ -f docs/version-manifest.json ]; then
  desfasados=$(node -e '
...
if [ "$falta" = "0" ]; then
  ...
  grep -oE 'amigable-shell-v[0-9]+' docs/sw.js | head -1 | sed 's/^/CACHE actual: /'
