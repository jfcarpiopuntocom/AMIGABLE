/*!
 * respaldo-empleado.js — Amigable-123
 * ============================================================================
 * QUE ES
 * ----------------------------------------------------------------------------
...
(function (global) {
  ...
  async function construirPaquete(desdeMs) {
    ...
    if (res.status === 403) {
      throw new Error("Este dispositivo todavía no está activado, así que no hay datos que respaldar.");
    }
    if (!res.ok) throw new Error("No se pudieron leer los datos.");
    ...
    return {
      texto: texto,
      nombre: "mi-trabajo-" + slug + "-" + stamp + ".json",
      humano: humano,
  ...
  async function correr(desdeMs, onMsg) {
    ...
    if (!bs || !bs.entregarArchivo) {
      msg("El módulo de respaldo no está cargado. Recarga la página e inténtalo de nuevo.", "#E8365D");
      return;
    }
    if (!prefs.canalEmail && !prefs.canalWhatsapp) { msg("Elige al menos un canal.", "#E8365D"); return; }
    if (prefs.canalEmail && !prefs.email) { msg("Escribe tu correo primero.", "#E8365D"); return; }
    if (prefs.canalWhatsapp && !bs.waEsValido(prefs.whatsapp)) {
      msg("Tu WhatsApp parece incompleto — inclúyelo con código de país.", "#E8365D"); return;
    }
    ...
    try {
      ...
      msg("No se pudo generar: " + e.message, "#E8365D");
      return;
    }

    var resultado = await bs.entregarArchivo(
      info, prefs, "Mi trabajo — amigable-123",
      "Constancia de mi trabajo (" + info.humano + "). Me la envío a mí por %CANAL%. Es mía y no pasa por ningún servidor."
    );
    ...
    msg("Listo. " + info.resumen.ventas + " venta(s) en tu constancia. Adjunta y envía.", "#00C87A");
  }
  ...
  var PERIODOS = [
    ...
    { key: "trimestre", label: "3 meses", dias: 92 },
  ...
  function montar(mount) {
    ...
    mount.innerHTML =
      ...
      + 'color:#2E6278 !important;-webkit-text-fill-color:#2E6278 !important;">Mi constancia de trabajo</h3>'
      + '<p style="margin:0 0 12px;font-size:15px;font-weight:700;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;">'
      + 'Mándate a ti mismo/a el registro de lo que vendiste y lo que te toca de comisión.</p>'
      + '<p style="margin:0 0 12px;font-size:15px;line-height:1.5;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;">'
      + 'Llega a tu correo o WhatsApp, con fecha y sellado. No pasa por ningún servidor nuestro. '
      + 'Incluye tus ventas, tus comisiones y tu actividad — no incluye costos ni datos de otras personas.</p>'
      + '<div style="display:grid;gap:10px;">'
      + '<div><div style="font-size:14px;font-weight:700;margin-bottom:6px;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;">Periodo</div>'
      ...
      + ' style="min-width:20px;min-height:20px;vertical-align:middle;margin-right:6px;">Enviarme por correo</label>'
      ...
      + ' style="min-width:20px;min-height:20px;vertical-align:middle;margin-right:6px;">Enviarme por WhatsApp</label>'
      + '<input type="tel" id="oc-re-wa" value="' + esc(prefs.whatsapp) + '" placeholder="+593 99 990 5080"'
      + ' style="padding:10px;border:2px solid #2E6278;border-radius:6px;min-height:44px;max-width:340px;font-size:15px;">'
      + '</div>'
      + '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:14px;">'
      + '<button id="oc-re-guardar" style="min-height:44px;padding:10px 16px;border:2px solid #2E6278;background:#fff;color:#2E6278;border-radius:8px;font-weight:700;font-size:15px;cursor:pointer;">Guardar mis datos</button>'
      + '<button id="oc-re-enviar" style="min-height:44px;padding:10px 16px;border:2px solid #2E6278;background:#2E6278;color:#fff;border-radius:8px;font-weight:700;font-size:15px;cursor:pointer;">Enviármelo ahora</button>'
      + '</div>'
