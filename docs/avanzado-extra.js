...
(function () {
  ...
  /* DOS LICENCIAS DEL MISMO CORREO (bug en vivo, JFC 2026-08-17: "mi licencia
     es AMG-7ZXZ-... pero me sale AMG-SA3L-... en Avanzados"). Bajo el rotulo
     "Tu licencia" se estaba mostrando la SALA de sincronizacion, que es otra
     cosa: la sala es donde se juntan los dispositivos, y puede haber quedado
     apuntando a una licencia vieja del mismo dueno.

     Si hay licencia propia registrada, ESA es la que se muestra siempre. La
     sala se dice aparte y con su nombre, y se ofrece corregirla de un toque.
     No se toca la sala sola: cambiarla desconectaria a los dispositivos que ya
     estan sincronizados en ella. */
  const licenciaMostrada = codigoPrecargado || salaActiva || "";
  const salaDistinta = !!(codigoPrecargado && salaActiva &&
    salaActiva.toUpperCase() !== codigoPrecargado.toUpperCase());
  panel.innerHTML = `
    <h3 class="seccion" style="margin-top:0;">Sincronizar equipo</h3>
    <p style="font-size:14px;color:var(--ink-soft);margin-top:0;">
      Todos los dispositivos de tu equipo (dueño, admins, encargados) que tengan
    ...
    <div id="oc-sync-activo" style="display:${salaActiva ? "block" : "none"};">
      ...
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
        ...
        ${salaDistinta ? `<div style="margin-top:10px;padding:10px;border:2px solid #E86040;border-radius:6px;">
          <p style="font-size:13px;margin:0 0 8px;color:var(--ink);"><strong>Tus dispositivos están sincronizados con otro código tuyo:</strong>
            <code style="background:var(--paper-deep,#E2E8ED);padding:2px 6px;border-radius:4px;">${escHtml(salaActiva)}</code>.
            Pasa si activaste dos licencias con el mismo correo. Tu licencia es la de arriba.</p>
          <button id="oc-sync-usar-propia" style="font-size:13px;padding:6px 12px;border-color:#E86040;color:#E86040;">Sincronizar con mi licencia (${escHtml(licenciaMostrada)})</button>
          <p style="font-size:12px;color:var(--ink-soft);margin:6px 0 0;">Los dispositivos que sigan en el código viejo dejarán de verse hasta que los pases también.</p>
        </div>` : ""}
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;">
        ...
        <!-- BOTONES DE SYNC PODADOS (JFC 2026-09-15, homologado de friendly-123):
             se BORRARON Resincronizar, Juntar inventario y Cambiar el código —
             aturdían y casi nadie los usaba. Quedan Compartir y Desactivar (más
             el botón condicional de "usar mi licencia" para el caso de licencia
             partida). Los handlers siguen guardados (if(!el) return) por si a
             futuro se reactivan. NO re-agregar sin pedido explícito de JFC. -->
  ...
  try {
    /* ENCENDIDO POR DEFECTO (JFC 2026-09-15): "0" explícito lo apaga; sin clave = on. */
    ...
    panel.insertAdjacentHTML("beforeend",
      ...
      '<p style="font-size:14px;color:#1a1a1a;margin:0 0 8px;">Mantiene al día a cada dispositivo del equipo, aparato con aparato, y junta todo sumando para no perder nada. Viene encendida; puedes apagarla aquí si alguna vez lo necesitas.</p>' +
  ...
  /* EXPORT / IMPORT — FORMA B: WHATSAPP (JFC 2026-09-15, corregido). Par CASADO
     ...
     descarga; abre WhatsApp. A PROPÓSITO NO incluye oc_secure (claves): un archivo
     que viaja por WhatsApp no debe llevar tus PIN. Import: cajita + botón; lee el
     envoltorio (o cualquier respaldo con .datos), CONFIRMA, POST /respaldo/importar,
     restaura fotos y avisa a la app. Soberano: nada toca un servidor nuestro. */
  try {
    panel.insertAdjacentHTML("beforeend",
      '<div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--azul-suave,#dde5ec);">' +
    ...
    if (_bx) _bx.addEventListener("click", async function () {
      try {
        ...
        if (!r.ok) { _copiaMsg(datos.error || "Activa este dispositivo (PIN 789) para exportar.", false); return; }
        ...
        var a = document.createElement("a"); a.href = url; a.download = "amigable-copia-" + stamp + ".json";
        ...
        var txt = encodeURIComponent("Aquí está mi copia de amigable-123 del " + stamp + ". Adjunto el archivo que se acaba de descargar — ábrelo en la app con Avanzado > Mover una copia > Importar.");
    ...
    if (_bi && _bif) {
      ...
      _bif.addEventListener("change", async function (e) {
        ...
        try {
          ...
          if (!paquete || !paquete.datos) { _copiaMsg("Este archivo no parece una copia de amigable-123.", false); return; }
  ...
  /* Mover la sala a la licencia propia. Accion explicita y avisada: nunca
     automatica, porque desconecta a quien siga en la sala vieja. */
  const btnUsarPropia = document.getElementById("oc-sync-usar-propia");
  if (btnUsarPropia) btnUsarPropia.addEventListener("click", async () => {
    const propia = _licenciaDeEstaApp();
    if (!propia) return;
    if (typeof ocConfirm === "function" && !(await ocConfirm(
      "Tus dispositivos van a pasar a la sala de " + propia + ". Los que sigan en el código " +
      "anterior dejarán de verse hasta que hagas lo mismo en cada uno. ¿Continuar?"))) return;
    btnUsarPropia.disabled = true;
    try { window.OCSyncControl.activar(propia); } catch (_) {}
    const msg = document.getElementById("oc-sync-msg");
    if (msg) { msg.style.color = "var(--sim-verde-dk,#1a6e3c)"; msg.textContent = "Sincronizando con " + propia + "…"; }
  ...
  /* TABLERO DE CONTROL (M13/M14/M15, 2026-08-15).
     ...
     que el tablero exige el codigo de la sala, que un encargado no tiene. Son
...
// atiende. Se pinta solo si quien mira es encargado — montar() se autolimita, no
  ...
  try {
    ...
    wrap.innerHTML =
      ...
      '<div id="oc-sync-faq" style="display:flex;flex-direction:column;gap:6px;margin:10px 0 18px;font-size:13px;color:var(--ink-soft);">' +
        '<div><strong style="color:var(--ink);">(?) Sincronizar equipo</strong> — mantiene a todos tus dispositivos (dueño, admins, encargados) al día en segundos, cifrado, para siempre mientras esté activo.</div>' +
    ...
    async function renderEmpleados() {
      ...
      equipo.forEach((u) => {
        ...
        if (puedeEditar) {
          ...
          // puede renombrar a un encargado pero no ascenderlo).
...
try{const syncDevPanel=document.createElement("div");syncDevPanel.id="oc-syncdev-panel";syncDevPanel.className="tag-card";syncDevPanel.style.cssText="text-align:left;margin-top:22px;";vista.appendChild(syncDevPanel);pintarSyncDev();}catch(e){console.error("Panel de sync entre dispositivos no cargo (aislado, no rompe Avanzado):",e)}
...
try{
  var HINTS={
    ...
    "Mi Equipo":"Encargados, roles y PINs de este negocio.",
  ...
  /* ICONOS DEL RIEL DE AVANZADO (JFC 2026-09-15, homologado de friendly-123).
     Un glifo por sección que SIGNIFICA lo que es. Sin emojis (regla dura de JFC):
     glifos Unicode monocromos de presentación de TEXTO, que no se vuelven emoji
     a color en iOS/Android/Safari. Claves = títulos en español de las secciones. */
  var ICONS={
    "Capa contable":"Σ",
    "Actividad reciente":"◷",
    "Gastos mensuales":"⊟",
