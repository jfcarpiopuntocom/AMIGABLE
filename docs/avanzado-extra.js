...
(function () {
  ...
  /* EXPORT — FORMA B: WHATSAPP (JFC 2026-09-15, homologado de friendly-123).
     Forma A = sync (arriba), Forma B = exportar una copia y compartirla por
     WhatsApp, Forma C (a futuro) = Loyverse. Soberano: descarga el respaldo
     (/respaldo/exportar) como archivo local y abre WhatsApp con un mensaje listo;
     el dueño adjunta el archivo recién bajado. No toca ningún servidor nuestro. */
  try {
    panel.insertAdjacentHTML("beforeend",
      '<div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--azul-suave,#dde5ec);">' +
    ...
    if (_bx) _bx.addEventListener("click", async function () {
      ...
      try {
        ...
        if (!r.ok) { if (_m) { _m.style.color = "var(--rojo,#a3392a)"; _m.textContent = datos.error || "Activa este dispositivo (PIN 789) para exportar."; } return; }
        var stamp = new Date().toISOString().slice(0, 10);
        var blob = new Blob([JSON.stringify(datos, null, 2)], { type: "application/json" });
        var url = URL.createObjectURL(blob);
        var a = document.createElement("a"); a.href = url; a.download = "amigable-respaldo-" + stamp + ".json";
        ...
        var txt = encodeURIComponent("Aquí está mi respaldo de amigable-123 del " + stamp + ". Adjunto el archivo que se acaba de descargar.");
