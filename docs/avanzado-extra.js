...
(function () {
  ...
  function init() {
    ...
    // Referencias que el riel (armado mas abajo) rellena, y que el listener
    // de desbloqueo de PIN (definido mas arriba, antes de que el riel exista
    // en el DOM) necesita para saltar a la pestaña "Capa contable" apenas se
    // mete el PIN correcto — antes quedaba desbloqueado pero visualmente
    // "perdido" en la pestaña que estuviera activa.
    let _rielActivarTab = null;
    let _rielIdContable = null;
    ...
    chartBox.innerHTML = `<h3 class="seccion" style="margin-top:0;">Location comparison (this month)</h3><p style="font-size:13px;color:var(--ink-soft);margin:-6px 0 10px;">Sales, margin, goal completion, and effective commission paid — one bar per location, so you can compare at a glance.</p><div id="oc-chart"></div>`;
    ...
    $("oc-acct-open").addEventListener("click", async () => {
      ...
      if (_rielActivarTab && _rielIdContable) _rielActivarTab(_rielIdContable);
      await render();
    });
    ...
    (function armarRielAvanzado() {
      try {
        ...
        const ETIQUETA_FIJA = { "oc-acct-lock": "Capa contable", "oc-contable": "Capa contable" };
        ...
        resto.forEach((nodo) => {
          ...
          if (titulo) {
            // Si ya existe una seccion con este MISMO titulo (caso "Capa
            // contable": el candado y el contenedor #oc-contable llegan
            // separados en el DOM pero son la misma cosa), se funden en una
            // sola pestaña del riel en vez de crear una entrada duplicada
            // o dejar el contenido mal etiquetado con un h3 interno cualquiera.
            const existente = secciones.find((s) => s.label === titulo);
            if (existente) {
              existente.nodos.push(nodo);
              actual = existente;
            } else {
        ...
        // "¿Cómo funciona?" corto por pestaña (JFC 2026-07-30, punto 4: "un
        // ¿Cómo funciona? por cada subseccion... una explicacion asi breve").
        // Data-driven a proposito: si una etiqueta no esta aca (secciones
        // nuevas, montadas por otro archivo), simplemente no lleva explicacion
        // - nunca se inventa una, mismo criterio que el resto del riel.
        const HINT_SECCION = {
          "Capa contable": "Cuentas T, ganancias y pérdidas, balance e inventario valorizado. Pedida aparte con una clave distinta a la del dueño porque es para el contador o para una revisión a fondo, no para el día a día.",
          "Recent activity": "Registro cronológico de ventas, ajustes y movimientos de todo el equipo — para auditar quién hizo qué y cuándo.",
          "Timezone": "Define qué cuenta como \"hoy\" para ventas, gastos y el cierre del día.",
          "Monthly expenses": "El gasto fijo mensual se reparte entre los días reales del mes para estimar el costo diario en el P&L.",
        };
        ...
        secciones.forEach((s) => {
          ...
          const hint = HINT_SECCION[s.label];
          if (hint) {
            const hEl = document.createElement("p");
            hEl.style.cssText = "font-size:13px;color:var(--ink-soft,#5d5340);margin:0 0 14px;padding-bottom:10px;border-bottom:1px solid var(--azul-suave,#dde5ec);";
        ...
        rielFila.style.cssText = "display:flex;gap:0;align-items:flex-start;margin:6px 0 18px;";

        const rielNav = document.createElement("nav");
        rielNav.id = "oc-riel-nav";
        // margin:0 explicito en el nav Y en cada boton (punto 3, JFC: "gap
        // antes del primer item de la izquierda que luce mal") - sin esto,
        // el margen por defecto del navegador en <nav>/<button> dejaba un
        // hueco irregular arriba del primer item segun el navegador.
        rielNav.style.cssText = "flex:0 0 auto;width:132px;display:flex;flex-direction:column;position:sticky;top:8px;border-right:2px solid var(--azul-suave,#dde5ec);padding-right:10px;padding-top:0;margin:0 14px 0 0;";
        rielNav.innerHTML = CATEGORIAS.map((c) =>
          `<button type="button" data-riel-tab="${c.id}" style="display:block;width:100%;text-align:left;background:none;border:none;border-left:3px solid transparent;margin:0;padding:10px 8px;font-size:14px;font-weight:700;cursor:pointer;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;">${c.label}</button>`
        ).join("");
        ...
        // Deja enganchado lo que el listener de desbloqueo de PIN (definido
        // mas arriba en init()) necesita para saltar aca apenas se mete el
        // PIN correcto, en vez de quedar desbloqueado pero "perdido" en
        // cualquier pestaña que estuviera activa.
        _rielActivarTab = activarTab;
        const seccionContable = secciones.find((s) => s.label === "Capa contable");
        if (seccionContable) _rielIdContable = seccionContable.id;
