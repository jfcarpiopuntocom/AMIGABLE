// borradores.js — AMIGABLE · Nada tecleado se pierde, nunca
// ============================================================================
...
(function () {
  ...
  /* Restaurar un borrador dispara input/change por campo para despertar a
     quien escuchaba (el checkbox que abre la fecha de caducidad, por ejemplo).
     Pero ESTE modulo tambien escucha esos eventos, asi que cada restauracion
     provocaba una tormenta de guardados del borrador recien restaurado. La
     bandera `restaurando` del enganche corta ese lazo (caza de bugs
     2026-08-18). */
  ...
  function enganchar(cont, nombre, opciones) {
    ...
    if (si) si.addEventListener("click", function () {
      ...
      try { aplicar(cont, previo.vals); } finally {
        /* Se suelta en el siguiente turno: los eventos sinteticos de aplicar()
           son sincronos, pero un listener de terceros puede reprogramar. */
...
(function () {
  ...
  var SEMILLA = [
    "Bar", "Cocina", "Bebidas sin alcohol", "Snacks",
    "Artesanía", "Arte y cuadros", "Joyería y bisutería",
    "Ropa", "Calzado", "Accesorios",
    "Cosmética y cuidado personal", "Hogar y decoración",
    "Papelería", "Juguetes", "Electrónica",
    "Herramientas y ferretería", "Mascotas",
    "Tickets y eventos", "Servicios",
  ];
