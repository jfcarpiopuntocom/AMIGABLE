...
var _amgEp="=YXZk5ycyV2ay92du8WawJXYjZmauMXYpNmblNWas1SZsJWYnlWbh9yL6MHc0RHa";var CF_WORKER_URL_DEFAULT=function(){try{return atob(_amgEp.split("").reverse().join(""))}catch(_){return""}}();/* NO CLOUD (JFC, regla dura, ver PRIVACY.md): esta funcion es EL UNICO lugar del codigo con permiso de mandar datos fuera del dispositivo, y SOLO estos campos: instanceId, licenseCode, email/nombre/apellido/cedula/whatsapp (todos opcionales, solo si el dueno los ingreso), y la accion (register/login/update). JAMAS productos, ventas, clientes, inventario, ni nada de negocio. Ver worker.js para el lado servidor de esta regla. *//* CORTACIRCUITOS (portado de friendly-123, patron de cockatiel escrito a mano: el manifiesto es sin dependencias). Sin esto, con el nodo caido cada llamada espera 8s de timeout y el panel se llena de la MISMA falla. Tras 5 fallos seguidos se pausa 5 min; un exito lo cierra. En memoria a proposito: recargar da otra oportunidad, que es lo que el usuario espera al recargar. */var _cbFallos=0,_cbAbiertoHasta=0;var CB_TOPE=5,CB_PAUSA_MS=5*60*1000;function _cbBloqueado(){return Date.now()<_cbAbiertoHasta}function _cbFallo(){_cbFallos++;if(_cbFallos>=CB_TOPE){_cbAbiertoHasta=Date.now()+CB_PAUSA_MS;_cbFallos=0;try{console.warn("[heartbeat] pausa 5 min tras "+CB_TOPE+" fallos")}catch(_){}}}function _cbExito(){_cbFallos=0;_cbAbiertoHasta=0}async function enviarHeartbeatLicencia(datos){try{if(_cbBloqueado())return;var url=(localStorage.getItem("amigable_cf_worker_url")||"").trim()||CF_WORKER_URL_DEFAULT;if(!url)return;var corta=function(v,max){if(v==null)return v;var s=String(v);return s.length>max?s.slice(0,max):s};var limpio={instanceId:corta(datos.instanceId,100),licenseCode:corta(datos.licenseCode,40),email:corta(datos.email,160),nombre:corta(datos.nombre,120),apellido:corta(datos.apellido,120),cedula:corta(datos.cedula,40),whatsapp:corta(datos.whatsapp,20),activatedAt:datos.activatedAt,accion:corta(datos.accion,30)};try{var _err=window.AMG&&window.AMG.Salud?window.AMG.Salud.paraEnviar():null;if(_err&&_err.length)limpio.errores=_err}catch(_){};var ctrl=new AbortController;var t=setTimeout(function(){ctrl.abort()},8e3);var res;try{res=await fetch(url.replace(/\/+$/,"")+"/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(limpio),signal:ctrl.signal})}finally{clearTimeout(t)}if(res&&res.ok){_cbExito()}else{_cbFallo()}try{if(res&&res.ok){try{if(limpio.errores&&window.AMG&&window.AMG.Salud)window.AMG.Salud.limpiar()}catch(_){}var r=await res.json();if(r&&typeof r.estado==="string"&&/^[a-z]{2,20}$/.test(r.estado)){var owned=JSON.parse(localStorage.getItem("amigable_owned")||"null")||{};owned.licenseEstado=r.estado;owned.licenseEstadoAt=Date.now();/* RESCATE DE LICENCIA: si el nodo tiene una y el dispositivo no, se adopta. SOLO rellena un hueco; el nodo nunca pisa al cliente. */try{var _lr=r.licenseCode;if(!owned.licenseCode&&typeof _lr==="string"&&/^AMG-[0-9A-Z*~$=-]{8,34}$/.test(_lr)){owned.licenseCode=_lr}}catch(_){}var payload=JSON.stringify(owned);try{localStorage.setItem("amigable_owned",payload)}catch(_){try{var rm2=[];for(var ii=0;ii<localStorage.length;ii++){var kk=localStorage.key(ii);if(kk&&kk.indexOf("vp_foto_percha_")===0)rm2.push(kk)}rm2.forEach(function(k2){try{localStorage.removeItem(k2)}catch(_){}});localStorage.setItem("amigable_owned",payload)}catch(_){}}}}}catch(_){}}catch(_){}}let demoSesion=false;let listo=window.OCSecure.migrarSiHaceFalta();const BLOQUEO_TRAS_INTENTOS=8;const BLOQUEO_DURACION_MS=60*1e3;function leerIntentos(){try{return JSON.parse(sessionStorage.getItem("oc_intentos"))||{fallos:0,bloqueadoHasta:0}}catch{return{fallos:0,bloqueadoHasta:0}}}function guardarIntentos(x){sessionStorage.setItem("oc_intentos",JSON.stringify(x))}function registrarFallo(){const st=leerIntentos();st.fallos+=1;const n=st.fallos;if(st.fallos>=BLOQUEO_TRAS_INTENTOS){st.bloqueadoHasta=Date.now()+BLOQUEO_DURACION_MS;st.fallos=0}guardarIntentos(st);return n}function registrarExito(){sessionStorage.removeItem("oc_intentos")}function msRestantesBloqueo(){const st=leerIntentos();return Math.max(0,st.bloqueadoHasta-Date.now())}const css=document.createElement("style");css.textContent=`\n  #oc-gate{position:fixed;inset:0;z-index:9999;background:var(--azul-oscuro,#1c3049);\n    display:flex;align-items:center;justify-content:center;padding:20px;\n    overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;overscroll-behavior-y:contain;}\n  #oc-gate .caja{background:var(--blanco-calido,#fbf5e8);border:2px solid var(--brass,#9c7a35);\n    border-radius:8px;padding:26px 22px;max-width:420px;width:100%;text-align:center;\n    margin:auto;flex:0 0 auto;}\n  #oc-gate h2{font-family:var(--font-display,sans-serif);color:var(--ink,#211c14);font-size:22px;margin:0 0 4px;}\n  #oc-gate .sub{font-size:14px;color:var(--ink-soft,#5d5340);margin-bottom:18px;}\n  .oc-slots{display:flex;gap:10px;justify-content:center;margin-bottom:16px;}\n  .oc-slots .slot{width:58px;height:58px;border:2px solid var(--azul-medio,#2c4a68);border-radius:6px;\n    display:flex;align-items:center;justify-content:center;font-size:26px;background:var(--crema,#f3e8cd);color:var(--ink,#211c14);}\n  .oc-slots .slot.lleno{border-color:var(--rust,#b2461f);}\n  .oc-pad{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;}\n  .oc-pad button{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;\n    padding:8px 4px;border:2px solid var(--ink,#211c14);border-radius:6px;background:var(--crema,#f3e8cd);\n    cursor:pointer;min-height:54px;}\n  .oc-pad button .dig{font-family:var(--font-display,sans-serif);font-weight:700;font-size:20px;color:var(--ink,#211c14);line-height:1;}\n  .oc-pad button .emo{font-size:13px;line-height:1;}\n  .oc-pad button:active{transform:translateY(1px);}\n  /* FIX 2026-07-07 (JFC: "se agrandan y arruinan todo"): digitar rapido el PIN\n     disparaba el double-tap zoom de iOS. touch-action:manipulation lo elimina\n     sin tocar el pinch-zoom de accesibilidad. */\n  #oc-gate button, .oc-subgate button{touch-action:manipulation;}\n  .oc-acciones{display:flex;gap:8px;margin-top:14px;}\n  .oc-acciones button{flex:1;font-family:var(--font-display,sans-serif);font-size:14px;padding:12px;\n    border-radius:6px;border:2px solid var(--azul-medio,#2c4a68);background:var(--blanco-calido,#fbf5e8);\n    color:var(--azul-medio,#2c4a68);cursor:pointer;min-height:44px;text-transform:uppercase;}\n  .oc-msg{min-height:20px;font-size:14px;font-weight:700;color:var(--rojo,#a3392a);margin-top:12px;}\n  #oc-gate.err .caja,.oc-subgate.err .caja{animation:ocshake .35s;}\n  @keyframes ocshake{0%,100%{transform:translateX(0)}25%{transform:translateX(-8px)}75%{transform:translateX(8px)}}\n  #oc-logout{font-family:var(--font-display,sans-serif);font-size:13px;padding:8px 12px;border-radius:5px;\n    border:2px solid var(--brass,#9c7a35);background:transparent;color:var(--blanco-calido,#fbf5e8);\n    cursor:pointer;text-transform:uppercase;}\n  /* FIX 2026-07-02: la vista se renombró de "liquidaciones" a "comisiones";\n     este selector seguía apuntando al data-vista viejo y el EMPLEADO veía el\n     botón Comisiones (datos financieros del dueño). Mantener sincronizado con\n     el data-vista del nav en index.html. */\n  body.rol-empleado nav button[data-vista="avanzado"],\n  body.rol-empleado nav button[data-vista="comisiones"]{display:none!important;}\n  /* Rol ADMIN (2026-07-22): puede ver avanzado pero NO cambiar clave/correo del dueno */\n  body.rol-admin #oc-c-owner{display:none!important;}\n  body.rol-admin #oc-email-edit,body.rol-admin #oc-email-save,body.rol-admin #oc-email-in{display:none!important;}\n  /* Rol CONTADOR (PIN 357 directo en el gate): solo ve la vista contable de solo lectura,\n     sin POS/inventario/clientes ni exportar/importar respaldo o caja fuerte local. */\n  body.rol-contador nav button:not([data-vista="contable"]){display:none!important;}\n  body.rol-contador #oc-exportar,\n  body.rol-contador label:has(#oc-importar-file),\n  body.rol-contador #oc-caja-guardar,\n  body.rol-contador #oc-caja-ver{display:none!important;}\n  #oc-acct-lock{text-align:center;padding:22px;}\n  #oc-acct-lock button{font-family:var(--font-display,sans-serif);font-size:14px;padding:12px 20px;\n    border-radius:6px;border:2px solid var(--rust,#b2461f);background:var(--rust,#b2461f);\n    color:var(--blanco-calido,#fbf5e8);cursor:pointer;min-height:44px;}\n  .oc-subgate{position:fixed;inset:0;z-index:9999;background:rgba(28,48,73,0.92);\n    display:flex;align-items:center;justify-content:center;padding:20px;\n    overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;overscroll-behavior-y:contain;}\n  .oc-subgate > .caja{margin:auto;flex:0 0 auto;}\n  /* Rol DEMO: ocultar cambio de claves y de correo (todo lo demás funciona) */\n  body.rol-demo #oc-clave-block, body.rol-demo #oc-email-edit,\n  body.rol-demo #oc-email-save, body.rol-demo #oc-email-in{display:none!important;}\n  /* FASE B3 (JFC 2026-08-13) - barra de conversion del demo.\n     Solo existe con body.rol-demo, y solo se muestra cuando el JS le pone\n     .visible (a los 30 s de sesion demo). Al cerrar sesion el body pierde\n     rol-demo y la barra desaparece sola: no hace falta limpiarla a mano. */\n  #oc-demo-barra{display:none;}\n  body.rol-demo #oc-demo-barra.visible{display:flex;position:fixed;left:0;right:0;bottom:0;z-index:9000;\n    align-items:center;justify-content:center;gap:12px;flex-wrap:wrap;\n    padding:12px 16px;background:#211c14;border-top:3px solid #E86040;}\n  #oc-demo-barra .txt{font-size:15px;font-weight:700;color:#FFFFFF !important;\n    -webkit-text-fill-color:#FFFFFF !important;margin:0;}\n  #oc-demo-barra button{font-family:var(--font-display,sans-serif);font-size:15px;font-weight:700;\n    min-height:44px;padding:11px 18px;border-radius:6px;border:2px solid #E86040;background:#E86040;\n    color:#FFFFFF !important;-webkit-text-fill-color:#FFFFFF !important;cursor:pointer;}\n  body.rol-demo.demo-barra-visible{padding-bottom:76px;}\n  @media (prefers-reduced-motion: no-preference){\n    body.rol-demo #oc-demo-barra.visible{animation:ocdemobar .28s ease-out;}\n    @keyframes ocdemobar{from{transform:translateY(100%)}to{transform:translateY(0)}}\n  }\n  `;document.head.appendChild(css);function montarTeclado(padEl,slotsEl,onComplete){let entrada=[];const pool=barajar(EMOJI_POOL);padEl.innerHTML="";for(let d=0;d<=9;d++){const b=document.createElement("button");b.dataset.d=String(d);b.innerHTML=`<span class="dig">${d}</span><span class="emo">${pool[d%pool.length]}</span>`;padEl.appendChild(b)}const slots=()=>slotsEl.querySelectorAll(".slot");function pintar(){slots().forEach((s,i)=>{if(entrada[i]!=null){s.textContent=String(entrada[i]);s.classList.add("lleno")}else{s.textContent="";s.classList.remove("lleno")}})}padEl._ocTeclado={entrada:()=>entrada,push:d=>entrada.push(d),pintar:pintar,onComplete:onComplete};if(!padEl.dataset.ocListenerMontado){padEl.dataset.ocListenerMontado="1";padEl.addEventListener("click",e=>{const st=padEl._ocTeclado;const b=e.target.closest("button[data-d]");if(!b||st.entrada().length>=3)return;st.push(Number(b.dataset.d));st.pintar();if(st.entrada().length===3){const code=st.entrada().join("");setTimeout(()=>st.onComplete(code),900)}})}pintar();return{reset:()=>{entrada=[];pintar()}}}const gate=document.createElement("div");gate.id="oc-gate";gate.innerHTML=`\n    <div class="caja">\n      <img src="./logo.png" alt="amigable-123" style="max-width:min(70vw,280px);width:auto;height:auto;margin:0 auto 6px;display:block;filter:drop-shadow(0 0 1.5px rgba(255,253,245,.55)) drop-shadow(0 0 3px rgba(232,160,32,.35));">\n      <p id=\"oc-gate-tagline\" style=\"margin:0 0 10px;font-size:13px;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;text-align:center;font-family:var(--font-mono,monospace);letter-spacing:.05em;\">Deja de adivinar. Empieza a ver.</p>
      ...
      <button type="button" id="oc-unirse-equipo" style="background:none;border:none;color:var(--azul-medio,#2c4a68) !important;-webkit-text-fill-color:var(--azul-medio,#2c4a68) !important;font-size:13px;text-decoration:underline;cursor:pointer;margin-top:10px;padding:6px;">¿Nuevo en este equipo? Únete con el código de tu negocio</button><button type="button" id="oc-gate-licencia" style="background:none;border:none;color:var(--azul-medio,#2c4a68) !important;-webkit-text-fill-color:var(--azul-medio,#2c4a68) !important;font-size:13px;text-decoration:underline;cursor:pointer;margin-top:6px;padding:6px;">Ver la licencia de este dispositivo</button><button type="button" id="oc-reidentificarme" style="background:none;border:none;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;font-size:13px;text-decoration:underline;cursor:pointer;margin-top:6px;padding:6px;">¿Eres el dueño y perdiste el acceso a este dispositivo? Reidentifícate</button>
      <p id=\"oc-gate-info\" style=\"margin:16px 0 0;font-size:13px;line-height:1.5;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;text-align:center;\">amigable-123 convierte la parte aburrida y abrumadora de manejar un negocio en algo vivo: tus productos hablan en colores que se encienden solos cuando toca actuar. Funciona offline, tus datos son solo tuyos, y no hay suscripciones ni anuncios de nadie. Tu negocio, a color.</p><div id=\"oc-gate-demo-pins\" style=\"margin:14px 0 0;padding:12px;border:1px dashed var(--azul-medio,#2c4a68);border-radius:6px;text-align:center;\"><p style=\"margin:0 0 6px;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--azul-medio,#2c4a68) !important;-webkit-text-fill-color:var(--azul-medio,#2c4a68) !important;\">Primera vez? Prueba con estos codigos</p><p style=\"margin:0;font-size:13px;line-height:1.6;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;\"><strong style=\"color:var(--ink,#211c14) !important;-webkit-text-fill-color:var(--ink,#211c14) !important;\">456</strong> demo &middot; <strong style=\"color:var(--ink,#211c14) !important;-webkit-text-fill-color:var(--ink,#211c14) !important;\">260</strong> encargado/a &middot; <strong style=\"color:var(--ink,#211c14) !important;-webkit-text-fill-color:var(--ink,#211c14) !important;\">357</strong> contador/a &mdash; o <strong style=\"color:var(--ink,#211c14) !important;-webkit-text-fill-color:var(--ink,#211c14) !important;\">789</strong> para activar tu propia instancia, gratuitamente, y empezar a usarla en serio.</p><p style=\"margin:8px 0 0;font-size:13px;line-height:1.5;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;\">Cada codigo muestra la app como la ve esa persona. El encargado no ve las ganancias.</p></div><p id=\"oc-gate-landing\" style=\"margin:12px 0 0;font-size:13px;line-height:1.5;text-align:center;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;\">&iquest;Llegaste aqui sin saber que es esto? <a href=\"./ahorra.html\" style=\"color:var(--azul-medio,#2c4a68) !important;-webkit-text-fill-color:var(--azul-medio,#2c4a68) !important;font-weight:700;text-decoration:underline;\">Mira de que se trata en 1 minuto</a>.</p>
      <div id="oc-gate-build" style="margin-top:8px;font-size:12px;font-family:var(--font-mono,monospace);letter-spacing:.04em;text-align:center;color:var(--ink,#211c14) !important;-webkit-text-fill-color:var(--ink,#211c14) !important;"></div><div id="oc-gate-lic4" style="display:none;margin-top:10px;font-size:13px;font-weight:700;letter-spacing:.06em;text-align:center;color:var(--ink,#211c14) !important;-webkit-text-fill-color:var(--ink,#211c14) !important;"></div>
      <button type="button" id="oc-gate-privacidad" style="background:none;border:none;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;font-size:13px;text-decoration:underline;cursor:pointer;margin-top:10px;padding:6px;">Política de Privacidad y Manejo de Datos</button>
    </div>`;document.body.appendChild(gate);/* LICENCIA A LA QUE UNO SE UNE, pasiva en el candado (JFC 2026-09-09, pedido de Belen: "que se vea a que me estoy uniendo, asi nadie se une a ciegas"; portado de friendly). SOLO los ultimos 4 caracteres — maxima privacidad, cero confusion. Alta legibilidad (tinta, no gris). Fail-safe: si algo falla, queda oculto. Lee la MISMA clave que "Ver la licencia de este dispositivo" (amigable_owned.licenseCode). */function pintarLic4Gate(){try{var el=document.getElementById("oc-gate-lic4");if(!el)return;var cod="";try{var o=JSON.parse(localStorage.getItem("amigable_owned")||"null")||{};if(/^AMG-/i.test(o.licenseCode||""))cod=String(o.licenseCode);}catch(_){}var s=cod.replace(/\s+/g,"");if(s.length<4){el.style.display="none";return;}el.textContent="Licencia ••••"+s.slice(-4);el.style.display="block";}catch(_){try{var e2=document.getElementById("oc-gate-lic4");if(e2)e2.style.display="none";}catch(__){}}}pintarLic4Gate();/* VERSION EN EL PIN (JFC 2026-09-09): v1.0 fija + shell real (entero). Lee version.json (que el SW nunca cachea). Alta legibilidad, tinta no gris. Fail-safe. */function pintarBuildGate(){try{fetch("./version.json?ts="+Date.now(),{cache:"no-store"}).then(function(r){return r.ok?r.json():null}).then(function(vj){if(!vj)return;var el=document.getElementById("oc-gate-build");if(!el)return;var shell=String(vj.shell||"").replace("amigable-shell-","");var ver=String(vj.version||"");el.textContent=(ver?("v"+ver):"")+(shell?("  ·  shell-"+shell):"")}).catch(function(){})}catch(_){}}pintarBuildGate();try{window.addEventListener("oc-negocio-actualizado",function(){try{pintarLic4Gate();}catch(_){}});}catch(_){}let teclado=null;let intervaloCountdown=null;function nuevoTeclado(){clearInterval(intervaloCountdown);const restante=msRestantesBloqueo();if(restante>0)return mostrarBloqueo(restante);$("oc-pad").style.display="";teclado=montarTeclado($("oc-pad"),$("oc-slots"),validar);$("oc-borrar").disabled=false}function mostrarBloqueo(msRestantes){$("oc-pad").style.display="none";$("oc-borrar").disabled=true;const pintar=()=>{const restante=msRestantesBloqueo();if(restante<=0){clearInterval(intervaloCountdown);nuevoTeclado();return}$("oc-msg").style.color="var(--rojo,#a3392a)";$("oc-msg").textContent=`Demasiados intentos. Espera ${Math.ceil(restante/1e3)}s.`};pintar();intervaloCountdown=setInterval(pintar,1e3)}function $(id){return document.getElementById(id)}function error(txt){$("oc-msg").style.color="var(--rojo,#a3392a)";$("oc-msg").textContent=txt;gate.classList.add("err");setTimeout(()=>gate.classList.remove("err"),400);nuevoTeclado()}async function validar(code){await listo;try{if(code===ACTIVATION_PIN&&!dispositivoApropiado()){registrarExito();return iniciarActivacion()}/* DEMO 888 (JFC, 2026-08-13): el 888 es el codigo que anunciamos en el gate, en la landing y en el checklist, asi que tiene que abrir el DEMO de verdad (rol-demo, encuadre de Fase B, barra de conversion) y no la app como dueno real. Antes de esto el 888 caia en verificarOwnerOEmpleado, que lo aceptaba como PIN de dueno por defecto: el visitante probaba la app entera sin ver nunca el encuadre ni el camino para activarla. El guard !dispositivoApropiado() protege a los duenos reales: al activar con 789 el PIN del dueno pasa a 789 y esta rama deja de correr en ese dispositivo. NO quitar esta linea sin mover tambien el copy de los 3 codigos demo. *//* 888 ya NO abre demo (2026-08-27): es PIN de dueño inicial libre; el demo es SOLO 456. */const sb=window.OCSecure.segundosBloqueo?window.OCSecure.segundosBloqueo("login"):0;if(sb>0){error(`Demasiados intentos. Espera ${sb}s e intenta de nuevo.`);return}const rolCombinado=await window.OCSecure.verificarOwnerOEmpleado(code);if(rolCombinado==="dueno"){registrarExito();return entrar("dueno")}if(rolCombinado==="empleado"){registrarExito();return entrar("empleado")}if(code===DEMO_PIN&&!dispositivoApropiado()){registrarExito();return entrar("demo")}if(await window.OCSecure.verificarAcct(code)){registrarExito();return entrar("contador")}const uNombrado=await verificarUsuarioNombrado(code);if(uNombrado){window.OCCurrentUser=uNombrado;registrarExito();return entrar(uNombrado.rol||"empleado")}const _nFallos=registrarFallo();const restante=msRestantesBloqueo();if(restante>0){error(`Demasiados intentos. Espera ${Math.ceil(restante/1e3)}s.`);return}if(window.OCSecure.estadoSecreto&&window.OCSecure.estadoSecreto()==="corrupto"){error("Los datos de acceso de este dispositivo se dañaron. Tus productos y ventas están intactos. Usa \"¿Olvidaste?\" para recuperar el acceso con tu correo.");return}if(_nFallos>=2){error("Clave incorrecta. ¿Estás probando la demo? El PIN de demo es 888.");return}error("Clave incorrecta. Intenta de nuevo.")}catch(e){try{registrarFallo()}catch(_){}error("No se pudo verificar la clave. Intenta de nuevo.")}}async function verificarUsuarioNombrado(pin){try{const r=await fetch("/api/usuarios/verificar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({pin:pin})});if(!r.ok)return null;return await r.json()}catch(_){return null}}let modalActivacion=null;function construirModalActivacion(){if(modalActivacion)return modalActivacion;var st=document.createElement("style");st.textContent=""+"#oc-act{position:fixed;inset:0;z-index:10010;background:#0F1923;display:flex;align-items:center;justify-content:center;padding:18px;}"+"#oc-act-card{background:#F8F9FB;width:100%;max-width:460px;border-radius:14px;border:2px solid #C4CDD8;border-top:4px solid #E86040;padding:26px 22px 24px;box-shadow:0 12px 40px #060d14;max-height:92vh;overflow-y:auto;}"+"#oc-act .marca{font-family:var(--font-mono,monospace);font-size:13px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#2E6278 !important;-webkit-text-fill-color:#2E6278 !important;margin:0 0 6px;}"+"#oc-act h2{font-family:var(--font-display,sans-serif);font-size:24px;font-weight:700;line-height:1.15;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;margin:0 0 10px;}"+"#oc-act p{font-family:var(--font-body,sans-serif);font-size:15px;line-height:1.5;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;margin:0 0 14px;}"+"#oc-act label.op{display:block;border:2px solid #C4CDD8;border-radius:10px;padding:12px 14px;margin:0 0 10px;cursor:pointer;background:#FFFFFF;}"+"#oc-act label.op input{margin-right:8px;}"+"#oc-act label.op strong{font-size:15px;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;}"+"#oc-act label.op span{display:block;font-size:14px;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;margin-top:2px;}"+"#oc-act .lbl{display:block;font-size:14px;font-weight:700;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;margin:14px 0 6px;}"+"#oc-act input[type=email],#oc-act input[type=text],#oc-act input[type=tel]{width:100%;box-sizing:border-box;padding:11px 12px;border:2px solid #5294AC;border-radius:8px;font-size:16px;font-family:var(--font-mono,monospace);color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;background:#FFFFFF;margin-bottom:2px;}"+"#oc-act .primario{width:100%;min-height:48px;margin-top:16px;padding:14px;border-radius:9px;border:2px solid #E86040;background:#E86040;color:#FFFFFF !important;-webkit-text-fill-color:#FFFFFF !important;font-size:16px;font-weight:700;cursor:pointer;}"+"#oc-act .secundario{width:100%;min-height:44px;margin-top:10px;padding:11px;border-radius:9px;border:2px solid #5294AC;background:transparent;color:#2E6278 !important;-webkit-text-fill-color:#2E6278 !important;font-size:15px;font-weight:700;cursor:pointer;}"+"#oc-act .msg{font-size:14px;font-weight:700;margin:10px 0 0;color:#B0183E !important;-webkit-text-fill-color:#B0183E !important;}"+"#oc-act .ok{color:#0F7A3D !important;-webkit-text-fill-color:#0F7A3D !important;}"+"@media (prefers-color-scheme: dark){#oc-act-card{background:#F8F9FB;}#oc-act h2,#oc-act p,#oc-act label.op strong,#oc-act .lbl,#oc-act input[type=email],#oc-act input[type=tel]{color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;}#oc-act label.op span{color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;}#oc-act .primario{color:#FFFFFF !important;-webkit-text-fill-color:#FFFFFF !important;}}";document.head.appendChild(st);var wrap=document.createElement("div");wrap.id="oc-act";wrap.innerHTML=""+'<div id="oc-act-card">'+'<div id="oc-act-form">'+'<p class="marca">amigable-123 &middot; activar mi negocio</p>'+"<h2>Este dispositivo pasa a ser tuyo</h2>"+"<p>De aqui en adelante, esta copia es tu negocio: tus productos, tus clientes, tus claves. Todo vive en tu dispositivo, sin nube ni intermediarios.</p>"+'<p style="font-weight:700;">Lo único que necesitas registrar es tu licencia del programa — todo el resto de tus datos permanece solamente contigo, por diseño.</p>'+'<p style="background:#FFF8E1;border:2px solid #E8A020;border-radius:8px;padding:10px 12px;font-size:14px;font-weight:700;color:#8A5A00;">Ojo: esto reemplaza el PIN de demo (888) por tu propio PIN. Si solo quieres seguir explorando la demo, toca &#39;Ahora no&#39;.</p>'+'<label class="op"><input type="radio" name="oc-act-datos" value="vaciar" checked><strong>Empezar vacio</strong><span>Quita los datos de ejemplo. Arrancas de cero con lo tuyo.</span></label>'+'<label class="op"><input type="radio" name="oc-act-datos" value="conservar"><strong>Conservar lo que ya cargue aqui</strong><span>Si ya metiste tus productos reales en este dispositivo, se quedan.</span></label>'+'<label class="lbl" for="oc-act-cedula">Cédula o pasaporte</label>'+'<input id="oc-act-cedula" type="text" inputmode="numeric" autocomplete="off" placeholder="1234567890">'+'<label class="lbl" for="oc-act-tel">Tu WhatsApp o tel\u00e9fono (obligatorio)</label>'+'<input id="oc-act-tel" type="tel" inputmode="tel" autocomplete="tel" placeholder="+593 99 123 4567">'+'<p style="font-size:13px;line-height:1.45;margin:4px 0 0;color:var(--ink-soft,#5d5340);">Es por donde te contactamos si algo pasa con tu licencia. Un correo rebota o se queda sin leer; un WhatsApp llega.</p>'+'<label class="lbl" for="oc-act-nombre">Primer nombre (o inicial)</label>'+'<input id="oc-act-nombre" type="text" autocomplete="given-name" placeholder="María">'+'<label class="lbl" for="oc-act-apellido">Primer apellido</label>'+'<input id="oc-act-apellido" type="text" autocomplete="family-name" placeholder="García">'+'<label class="lbl" for="oc-act-email">Correo (para recuperar el acceso si olvidas el PIN)</label>'+'<input id="oc-act-email" type="email" inputmode="email" autocomplete="email" placeholder="tucorreo@dominio.com">'+'<label class="lbl" for="oc-act-licencia-existente" style="margin-top:14px;">&iquest;Ya activaste amigable-123 en otro dispositivo? Pon tu c&oacute;digo de licencia aqu&iacute; en vez de crear uno nuevo (opcional)</label>'+'<input id="oc-act-licencia-existente" type="text" autocomplete="off" placeholder="AMG-XXXX-XXXX-XXXX-XXXXX" style="text-transform:uppercase;">'+'<button id="oc-act-confirmar" class="primario">Activar mi negocio</button>'+'<button id="oc-act-cancelar" class="secundario">Ahora no</button>'+'<p id="oc-act-msg" class="msg"></p>'+"</div>"+'<div id="oc-act-exito" style="display:none;">'+'<p class="marca">amigable-123 &middot; listo</p>'+"<h2>Ya tienes tu cuaderno digital compartido</h2>"+'<p style="font-size:16px;line-height:1.5;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;margin:0 0 14px;">'+"Acabas de hacer algo que la mayoria de negocios pequenos todavia no hace: sacar el inventario de la cabeza y de los cuadernos, y ponerlo donde tu equipo lo pueda ver."+"</p>"+'<p style="font-size:15px;line-height:1.5;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;background:#F8F9FB;border-left:4px solid #00C87A;border-radius:0 8px 8px 0;padding:12px 14px;margin:0 0 16px;">'+"Un punto de venta cobra. Un contable declara. Ninguno de los dos te dice, un martes cualquiera, que producto se esta muriendo en la percha ni quien movio el stock. Eso es lo que empieza hoy."+"</p>"+'<p id="oc-act-exito-txt"></p>'+'<div style="background:#F8F9FB;border:1px solid #E8ECF2;border-radius:10px;padding:14px;margin:16px 0;text-align:left;">'+'<p style="font-size:15px;font-weight:700;color:#0F1923 !important;-webkit-text-fill-color:#0F1923 !important;margin:0 0 8px;">Los primeros 15 minutos</p>'+'<p style="font-size:15px;line-height:1.5;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;margin:0;">1) Carga 10 productos que muevas seguido. 2) Registra una venta de prueba. 3) Mira como cambia el color. Con eso ya entendiste el sistema completo.</p>'+"</div>"+'<p style="font-size:15px;line-height:1.5;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;margin:0 0 16px;">No hay cuenta regresiva. Mientras tengas hasta 25 productos, 100 ventas al mes y 1 encargado, sigue siendo gratis el tiempo que sea.</p>'+'<button id="oc-act-entrar" class="primario">Empezar con mis primeros productos</button>'+'<p style="font-size:14px;line-height:1.5;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;margin:14px 0 0;">Si algo no cuadra, escribe por WhatsApp al <a href="https://wa.me/593999905080" target="_blank" rel="noopener" style="color:#2E6278 !important;-webkit-text-fill-color:#2E6278 !important;font-weight:700;">+593 99 990 5080</a>. Contesta la misma persona que construyo esto, durante los 5 anos de tu licencia.</p>'+'<p style="font-size:14px;margin:8px 0 0;"><a href="./manual-maestro.html" style="color:#2E6278 !important;-webkit-text-fill-color:#2E6278 !important;">Ver el recorrido completo en el manual</a></p>'+"</div>"+"</div>";document.body.appendChild(wrap);try{_ocMascaraLicencia(wrap.querySelector("#oc-act-licencia-existente"))}catch(_){};modalActivacion=wrap;var emailIn=wrap.querySelector("#oc-act-email");var msgEl=wrap.querySelector("#oc-act-msg");function setMsg(t,ok){msgEl.textContent=t;msgEl.className=ok?"msg ok":"msg"}wrap.querySelector("#oc-act-cancelar").addEventListener("click",function(){wrap.style.display="none"});wrap.querySelector("#oc-act-confirmar").addEventListener("click",async function(){var email=(emailIn.value||"").trim();if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)){setMsg("Escribe un correo valido — es tu unica forma de recuperar el acceso si olvidas el PIN.");emailIn.focus();return}var _telIn=wrap.querySelector("#oc-act-tel");var _telCrudo=(_telIn&&_telIn.value||"").trim();var telDigitos=_telCrudo.replace(/\D/g,"");/* TELEFONO OBLIGATORIO (JFC 2026-08-19, regla para TODAS sus apps). Un correo rebota, cae en spam o no se lee en semanas; un WhatsApp llega. El telefono es MAS importante que la cedula. Se valida a mano y NO se depende de intl-tel-input: esa libreria viene de un CDN y un campo obligatorio de la activacion no puede quedar atado a que un CDN responda. 7 a 15 digitos: el minimo de un numero local corto y el maximo de E.164. */if(telDigitos.length<7||telDigitos.length>15){setMsg("Escribe un telefono valido (7 a 15 digitos, con codigo de pais si quieres).");if(_telIn)_telIn.focus();return;}var cedula=(wrap.querySelector("#oc-act-cedula")?wrap.querySelector("#oc-act-cedula").value:"").trim();var nombre=(wrap.querySelector("#oc-act-nombre")?wrap.querySelector("#oc-act-nombre").value:"").trim();var apellido=(wrap.querySelector("#oc-act-apellido")?wrap.querySelector("#oc-act-apellido").value:"").trim();var vaciar=(wrap.querySelector('input[name="oc-act-datos"]:checked')||{}).value!=="conservar";var btn=wrap.querySelector("#oc-act-confirmar");btn.disabled=true;setMsg("Activando...",true);var idInstancia=globalThis.crypto&&globalThis.crypto.randomUUID?globalThis.crypto.randomUUID():Date.now().toString(36)+"-"+Math.random().toString(36).slice(2);var _licExistente=(wrap.querySelector("#oc-act-licencia-existente")?wrap.querySelector("#oc-act-licencia-existente").value:"").trim().toUpperCase();if(_licExistente&&!/^AMG-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9*~$=]{5}$/.test(_licExistente)){btn.disabled=false;setMsg("Ese codigo de licencia no tiene el formato AMG-XXXX-XXXX-XXXX-XXXXX. Revisalo (o borralo si de verdad quieres una licencia nueva) antes de continuar - si seguimos con un codigo mal escrito, se te crea una licencia NUEVA sin avisarte.");var _licIn=wrap.querySelector("#oc-act-licencia-existente");if(_licIn)_licIn.focus();return}
    // UNA LICENCIA POR PERSONA (JFC 2026-07-30): "no solo avisar, solo puede
    // haber una licencia por correo y por cedula". Antes de generar/aceptar
    // un codigo, se pregunta al worker si ese correo o cedula ya tienen una
    // licencia en otro lado. Si la hay, se REUSA esa (nunca se crea una
    // segunda) - asi ni un codigo mal pegado ni un olvido pueden duplicar
    // la licencia de un mismo dueno. Fail-open: si no hay conexion, sigue
    // normal (no bloquea una activacion legitima por un hipo de red).
    if(!_licExistente){
      try{
        var _baseDup=_ocWorkerBase();
        if(_baseDup){
          var _rDup=await fetch(_baseDup+"/verificar-duplicado",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:email,cedula:cedula,instanceId:idInstancia})});
          var _jDup=await _rDup.json().catch(function(){return{}});
          if(_jDup&&_jDup.duplicado&&_jDup.licenseCode){
            _licExistente=_jDup.licenseCode;
            setMsg("Ya tienes una licencia registrada con este "+(_jDup.por==="cedula"?"numero de cedula/pasaporte":"correo")+": "+_licExistente+". La estamos usando en vez de crear una nueva.",true);
          }
        }
      }catch(_){}
    }
    var licenseCode=_licExistente||generarCodigoAMG();try{if(window.OCSyncControl)window.OCSyncControl.activar(licenseCode)}catch(_){}try{await fetch("/api/instancia/activar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({vaciar:vaciar,instanceId:idInstancia})})}catch(_){}var _pinGuardado=false;try{_pinGuardado=await window.OCSecure.fijarOwnerPin("789")}catch(_){}if(!_pinGuardado){btn.disabled=false;setMsg("No se pudo activar (memoria del dispositivo llena). Libera espacio y toca \"Activar mi negocio\" de nuevo - nada quedo a medias.");return}try{window.OCSecure.actualizarCorreo(email)}catch(_){}if(vaciar){try{var rm=[];for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k&&k.indexOf("vp_foto_percha_")===0)rm.push(k)}rm.forEach(function(kk){localStorage.removeItem(kk)})}catch(_){}}var ownedData={instanceId:idInstancia,email:email,whatsapp:telDigitos,activatedAt:Date.now(),licenseCode:licenseCode};try{if(window.OCSecure&&window.OCSecure.actualizarWhatsapp)window.OCSecure.actualizarWhatsapp(telDigitos)}catch(_){}if(cedula)ownedData.cedula=cedula;if(nombre)ownedData.nombre=nombre;if(apellido)ownedData.apellido=apellido;var _ownedPayload=JSON.stringify(ownedData);try{localStorage.setItem("amigable_owned",_ownedPayload)}catch(_){try{var rmO=[];for(var io=0;io<localStorage.length;io++){var ko=localStorage.key(io);if(ko&&ko.indexOf("vp_foto_percha_")===0)rmO.push(ko)}rmO.forEach(function(kk){try{localStorage.removeItem(kk)}catch(_){}});localStorage.setItem("amigable_owned",_ownedPayload)}catch(_){}}/* NO marcar amigable_bienvenida_v3 aqui — el wizard debe mostrarse de verdad tras el primer login post-activacion (ver welcome-ui.js). Bug anterior: se marcaba "vista" en este punto sin que el usuario la viera nunca. */registrarExito();try{if(window.OCStorageDurable)window.OCStorageDurable.verificarYSolicitar()}catch(_){};enviarHeartbeatLicencia({instanceId:idInstancia,licenseCode:licenseCode,email:email,nombre:nombre,apellido:apellido,cedula:cedula,whatsapp:telDigitos,activatedAt:ownedData.activatedAt,accion:"register"});var seguro=email.replace(/[&<>"']/g,"");var codigoHtml=licenseCode.replace(/[&<>"']/g,"");wrap.querySelector("#oc-act-exito-txt").innerHTML="<p style='background:#F8F9FB;border:1px solid #E8ECF2;border-radius:8px;padding:10px 12px;margin:0 0 14px;font-size:15px;color:#0F1923;'>Desde ahora tu PIN es <strong>789</strong> y el de demo (888) ya no abre este dispositivo. Cambialo cuando quieras en Avanzado &rarr; Claves.</p>"+"Correo guardado: <strong>"+seguro+"</strong><br><br>"+"Tu c&oacute;digo de licencia: <strong style='font-family:monospace;letter-spacing:.1em;font-size:18px;color:#E86040;'>"+codigoHtml+"</strong><br>"+"<small style='color:#2C3E50;font-size:14px;'>Gu&aacute;rdalo ahora: m&aacute;ndatelo por WhatsApp a ti mismo o s&aacute;cale una foto. Con este c&oacute;digo te reconocemos si necesitas ayuda o si cambias de tel&eacute;fono. Aparece tambi&eacute;n en Ayuda (?).</small><p style='margin:10px 0 0;font-size:15px;line-height:1.5;color:#0F1923;background:#FFF6F2;border-left:4px solid #E86040;border-radius:0 8px 8px 0;padding:10px 12px;'><strong>Este c&oacute;digo es privado de tu equipo.</strong> Quien lo tiene, entra a tus datos. Ap&uacute;ntalo en un lugar seguro y comp&aacute;rtelo solo dentro de tu equipo, uno a uno. As&iacute; no tienes que cambiarlo despu&eacute;s.</p>";wrap.querySelector("#oc-act-form").style.display="none";wrap.querySelector("#oc-act-exito").style.display="block"});wrap.querySelector("#oc-act-entrar").addEventListener("click",function(){wrap.style.display="none";entrar("dueno")});return wrap}function iniciarActivacion(){var w=construirModalActivacion();w.querySelector("#oc-act-form").style.display="block";w.querySelector("#oc-act-exito").style.display="none";w.querySelector("#oc-act-msg").textContent="";w.querySelector("#oc-act-confirmar").disabled=false;w.style.display="flex";setTimeout(function(){var e=w.querySelector("#oc-act-email");if(e)e.focus()},80)}function entrar(nuevoRol){if(nuevoRol!=="demo"){try{var owned=JSON.parse(localStorage.getItem("amigable_owned")||"null")||{};if(owned.licenseEstado==="bloqueada"){error("Esta instancia está bloqueada. Contacta al administrador de amigable-123.");return}}catch(_){}}const esDemo=nuevoRol==="demo";demoSesion=esDemo;rol=esDemo?"dueno":nuevoRol;document.body.classList.toggle("rol-empleado",rol==="empleado");document.body.classList.toggle("rol-dueno",rol==="dueno");document.body.classList.toggle("rol-demo",esDemo);document.body.classList.toggle("rol-contador",rol==="contador");document.body.classList.toggle("rol-admin",rol==="admin");gate.style.display="none";document.body.style.overflow="";if(document.activeElement&&document.activeElement.blur)document.activeElement.blur();window.scrollTo(0,0);montarLogout();reiniciarInactividad();if(!esDemo&&rol==="dueno"&&dispositivoApropiado()&&window.OCSecure&&window.OCSecure.tieneOwnerPassword&&!window.OCSecure.tieneOwnerPassword()){setTimeout(function(){try{pedirPasswordInicial()}catch(_){}},700)}if(rol==="empleado"||rol==="admin"){const n=document.querySelector('nav button[data-vista="hoy"]');if(n)n.click()}if(!esDemo){try{var ow=JSON.parse(localStorage.getItem("amigable_owned")||"null")||{};if(ow.instanceId){enviarHeartbeatLicencia({instanceId:ow.instanceId,licenseCode:ow.licenseCode,email:ow.email,nombre:ow.nombre,apellido:ow.apellido,cedula:ow.cedula,whatsapp:ow.whatsapp||"",activatedAt:ow.activatedAt,accion:"login"})}}catch(_){}}window.dispatchEvent(new CustomEvent("oc-login",{detail:{rol:rol,demo:esDemo}}));if(rol==="contador"){const nc=document.querySelector('nav button[data-vista="contable"]');if(nc)nc.click()}if(esDemo){try{mostrarBienvenidaDemo()}catch(_){}try{programarBarraDemo()}catch(_){}}}/* ============================================================================
   FASE B - encuadre y conversion de la sesion demo (JFC, 2026-08-13).
   Autorizado por JFC. NO BORRAR sin avisarle: la barra de abajo es la UNICA
   via de conversion que existe dentro del demo. Antes de esto, alguien podia
   probar la app entera, quedar convencido, y no tener ningun camino hacia
   activarla.

   B1  Modal de bienvenida, una sola vez por sesion. La marca vive en
       sessionStorage, que aislamiento.js ya prefija por app ("amig::"), asi
       que no colisiona con friendly-123 ni consultorio-123 en el mismo origen.
   B2/B5/B6/B7 son parrafos dentro de ese mismo modal: primera accion sugerida,
       nombre de la categoria, limite real del demo, y salida a la calculadora
       para el perfil que decide por numeros.
   B3  Barra fija inferior. Aparece a los B3_DELAY_MS de sesion demo. El umbral
       de 30 s no es arbitrario: la probabilidad de abandono de una sesion es
       maxima en los primeros 10 s y se aplana alrededor de los 30 s. Antes de
       eso la barra interrumpe a alguien que todavia esta decidiendo si se
       queda; mucho despues, ya no llega a tiempo. Para moverlo, cambia solo
       B3_DELAY_MS.
   B4  Confirmacion antes de salir del demo hacia la activacion, para que nadie
       toque el boton temiendo que sea irreversible o largo.
   ============================================================================ */
var B3_DELAY_MS = 30 * 1e3;
var DEMO_BIENVENIDA_KEY = "oc_demo_bienvenida_vista";
var _demoBarraTimer = null;

function mostrarBienvenidaDemo(){
  try{ if(sessionStorage.getItem(DEMO_BIENVENIDA_KEY)) return; }catch(_){}
  var cont = _ocSubgate("oc-demo-bienvenida");
  if(!cont) return;
  try{ sessionStorage.setItem(DEMO_BIENVENIDA_KEY, "1") }catch(_){}
  cont.innerHTML =
    '<div class="caja" style="background:var(--blanco-calido,#fbf5e8);border:2px solid var(--brass,#9c7a35);'
    + 'border-radius:8px;padding:26px 22px;max-width:460px;width:100%;text-align:left;">'
    + '<h2 style="font-family:var(--font-display,sans-serif);color:#211c14 !important;'
    + '-webkit-text-fill-color:#211c14 !important;font-size:22px;margin:0 0 12px;">Est&aacute;s viendo un negocio de ejemplo</h2>'
    + '<p style="font-size:16px;line-height:1.5;color:#211c14 !important;-webkit-text-fill-color:#211c14 !important;margin:0 0 12px;">'
    + 'Los productos, las ventas y los clientes que ver&aacute;s son inventados, para que puedas tocar todo sin miedo a romper nada. '
    + 'Vende, ajusta stock, borra lo que quieras.</p>'
    + '<p style="font-size:16px;line-height:1.5;color:#211c14 !important;-webkit-text-fill-color:#211c14 !important;'
    + 'background:#FFFFFF;border-left:4px solid #E86040;border-radius:0 8px 8px 0;padding:12px 14px;margin:0 0 12px;">'
    + 'Empieza por Inventario: los colores te van a decir qu&eacute; hace falta hacer hoy, sin que leas un solo n&uacute;mero.</p>'
    + '<p style="font-size:16px;line-height:1.5;color:#211c14 !important;-webkit-text-fill-color:#211c14 !important;margin:0 0 12px;">'
    + 'Lo &uacute;nico que el demo no hace es guardar tu negocio real. Para eso est&aacute; el c&oacute;digo <strong>789</strong>, que es gratis.</p>'
    + '<p style="font-size:16px;line-height:1.5;color:#211c14 !important;-webkit-text-fill-color:#211c14 !important;margin:0 0 18px;">'
    + 'Esto no es un punto de venta ni un programa de contabilidad. Es un cuaderno digital compartido: lo que t&uacute; y tu equipo '
    + 'necesitan mirar todos los d&iacute;as.</p>'
    + '<button id="oc-demo-bv-ok" style="width:100%;font-family:var(--font-display,sans-serif);font-size:16px;font-weight:700;'
    + 'min-height:48px;padding:13px;border-radius:6px;border:2px solid #E86040;background:#E86040;color:#FFFFFF !important;'
    + '-webkit-text-fill-color:#FFFFFF !important;cursor:pointer;">Entendido, quiero probar</button>'
    + '<p style="font-size:14px;line-height:1.5;text-align:center;margin:14px 0 0;color:#2C3E50 !important;'
    + '-webkit-text-fill-color:#2C3E50 !important;">&iquest;Prefieres ver primero cu&aacute;nto tiempo recuperas? '
    + '<a href="./ahorra.html#calculadora" style="color:#E86040 !important;-webkit-text-fill-color:#E86040 !important;'
    + 'font-weight:700;">Est&aacute; calculado aqu&iacute;</a>.</p>'
    + '</div>';
  document.body.appendChild(cont);
  cont.querySelector("#oc-demo-bv-ok").addEventListener("click", function(){ cont.cerrar() });
}

/* B3 - la barra se arma una sola vez y se revela pasado el umbral. El chequeo
   de rol-demo dentro del timer evita que aparezca si la persona cerro sesion
   o activo su instancia mientras corria el reloj. */
function programarBarraDemo(){
  if(_demoBarraTimer){ clearTimeout(_demoBarraTimer); _demoBarraTimer = null; }
  var barra = document.getElementById("oc-demo-barra");
  if(!barra){
    barra = document.createElement("div");
    barra.id = "oc-demo-barra";
    barra.innerHTML = '<p class="txt">Est&aacute;s en modo demo, con datos de ejemplo.</p>'
      + '<button id="oc-demo-activar">Empezar con mi negocio, gratis</button>';
    document.body.appendChild(barra);
    barra.querySelector("#oc-demo-activar").addEventListener("click", confirmarSalidaDemo);
  }
  _demoBarraTimer = setTimeout(function(){
    if(!document.body.classList.contains("rol-demo")) return;
    barra.classList.add("visible");
    document.body.classList.add("demo-barra-visible");
  }, B3_DELAY_MS);
}

/* B4 - decir que pasa al tocar el boton. Al confirmar solo cerramos la sesion
   demo y dejamos el gate listo: la activacion real la sigue haciendo el flujo
   de 789 de siempre, aqui no se duplica ninguna logica de activacion. */
function confirmarSalidaDemo(){
  var cont = _ocSubgate("oc-demo-conf");
  if(!cont) return;
  cont.innerHTML =
    '<div class="caja" style="background:var(--blanco-calido,#fbf5e8);border:2px solid var(--brass,#9c7a35);'
    + 'border-radius:8px;padding:26px 22px;max-width:440px;width:100%;text-align:left;">'
    + '<h2 style="font-family:var(--font-display,sans-serif);color:#211c14 !important;'
    + '-webkit-text-fill-color:#211c14 !important;font-size:21px;margin:0 0 12px;">Empezar a usar la app para mi negocio</h2>'
    + '<p style="font-size:16px;line-height:1.5;color:#211c14 !important;-webkit-text-fill-color:#211c14 !important;margin:0 0 18px;">'
    + 'Vas a empezar a usar la app para tu negocio, en blanco, en este mismo dispositivo. Los datos de ejemplo desaparecen. '
    + 'Toma menos de un minuto.</p>'
    + '<button id="oc-demo-conf-ok" style="width:100%;font-family:var(--font-display,sans-serif);font-size:16px;font-weight:700;'
    + 'min-height:48px;padding:13px;border-radius:6px;border:2px solid #E86040;background:#E86040;color:#FFFFFF !important;'
    + '-webkit-text-fill-color:#FFFFFF !important;cursor:pointer;">S&iacute;, empezar con mi negocio</button>'
    + '<button id="oc-demo-conf-no" style="width:100%;margin-top:10px;background:none;border:none;font-size:15px;'
    + 'min-height:44px;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;cursor:pointer;">Seguir mirando el demo</button>'
    + '</div>';
  document.body.appendChild(cont);
  cont.querySelector("#oc-demo-conf-no").addEventListener("click", function(){ cont.cerrar() });
  cont.querySelector("#oc-demo-conf-ok").addEventListener("click", function(){
    cont.cerrar();
    if(_demoBarraTimer){ clearTimeout(_demoBarraTimer); _demoBarraTimer = null; }
    var b = document.getElementById("oc-demo-barra");
    if(b) b.classList.remove("visible");
    document.body.classList.remove("demo-barra-visible");
    cerrarSesion();
    try{
      var m = $("oc-msg");
      m.style.color = "var(--ink-soft,#5d5340)";
      m.textContent = "Escribe 789 para activar tu negocio.";
    }catch(_){}
  });
}

const INACTIVIDAD_MS=30*60*1e3;let temporizadorInactividad=null;function reiniciarInactividad(){clearTimeout(temporizadorInactividad);if(!rol)return;temporizadorInactividad=setTimeout(()=>cerrarSesion("Sesión cerrada por inactividad."),INACTIVIDAD_MS)}document.addEventListener("click",reiniciarInactividad);document.addEventListener("keydown",reiniciarInactividad);function cerrarSesion(mensaje){clearTimeout(temporizadorInactividad);rol=null;demoSesion=false;window.OCCurrentUser=null;document.body.classList.remove("rol-empleado","rol-dueno","rol-demo","rol-contador","rol-admin");nuevoTeclado();gate.style.display="flex";document.body.style.overflow="hidden";$("oc-msg").style.color=mensaje?"var(--rojo,#a3392a)":"";$("oc-msg").textContent=mensaje||"";const b=document.getElementById("oc-logout");if(b)b.remove();const chipViejo=document.getElementById("oc-user-chip");if(chipViejo)chipViejo.remove();window.dispatchEvent(new CustomEvent("oc-logout"))}$("oc-borrar").addEventListener("click",()=>{$("oc-msg").textContent="";if(teclado)teclado.reset()});$("oc-recuperar").addEventListener("click",(ev)=>{var b=ev.currentTarget;if(b.dataset.ocBusy)return;b.dataset.ocBusy="1";setTimeout(function(){delete b.dataset.ocBusy},1500);abrirFlujoReset()});$("oc-unirse-equipo").addEventListener("click",()=>abrirUnirseEquipo());$("oc-reidentificarme").addEventListener("click",()=>abrirReidentificarme());$("oc-gate-privacidad").addEventListener("click",()=>abrirPoliticaPrivacidad());
/* VER LA LICENCIA SIN ENTRAR (JFC, 2026-08-15, tras quedarse fuera de la suya).
   La licencia se generaba al activar y despues solo se veia dentro de Avanzado,
   o sea DETRAS del PIN. Quien olvidaba el PIN perdia tambien el rastro de su
   licencia, que es lo que hace falta para el tablero y para unir otro equipo.

   Por que esto NO es un agujero: la licencia ya esta guardada EN ESTE
   dispositivo, en claro. Cualquiera con el aparato en la mano puede leerla
   desde la consola del navegador en diez segundos. Ocultarla detras del PIN no
   protegia nada; solo dejaba fuera al duenio legitimo. El PIN sigue protegiendo
   lo que importa: los datos del negocio.

   Se muestra SOLO la de esta app (ver aislamiento.js) y nunca un PIN. */
$("oc-gate-licencia").addEventListener("click",function(){
  var cod="";
  try{var o=JSON.parse(localStorage.getItem("amigable_owned")||"null")||{};
    if(/^AMG-/i.test(o.licenseCode||""))cod=o.licenseCode;}catch(_){}
  var m=document.createElement("div");m.className="oc-subgate";
  m.innerHTML='<div class="caja" style="max-width:400px;">'+
    (cod
      ? '<h2 style="font-size:19px;margin:0 0 8px;">La licencia de este dispositivo</h2>'+
        '<p style="font-size:14px;line-height:1.5;color:var(--ink-soft,#5d5340);margin:0 0 12px;">An&oacute;tala en un lugar seguro. La necesitas para el tablero de control y para sumar otro equipo.</p>'+
        '<code style="display:block;font-size:17px;font-weight:700;letter-spacing:.06em;background:var(--crema,#f3e8cd);color:var(--ink,#211c14);padding:12px;border-radius:6px;word-break:break-all;">'+cod+'</code>'+
        '<button type="button" id="oc-gl-copiar" class="ir" style="width:100%;margin-top:12px;min-height:44px;">Copiar</button>'
      : '<h2 style="font-size:19px;margin:0 0 8px;">Este dispositivo no tiene licencia</h2>'+
        '<p style="font-size:15px;line-height:1.55;margin:0 0 12px;">Todav&iacute;a no se ha activado aqu&iacute;, o se activ&oacute; en otro. Si ya tienes tu licencia, &uacute;nete con ella desde &quot;&iquest;Nuevo en este equipo?&quot;.</p>')+
    '<p id="oc-gl-msg" style="font-size:14px;font-weight:700;margin:10px 0 0;min-height:19px;color:var(--verde,#2f7a4f);"></p>'+
    '<button type="button" id="oc-gl-cerrar" style="width:100%;margin-top:10px;min-height:44px;border-radius:6px;border:2px solid var(--azul-medio,#2c4a68);background:transparent;color:var(--azul-medio,#2c4a68);font-size:15px;font-weight:700;cursor:pointer;">Cerrar</button></div>';
  ...
  document.getElementById("oc-gl-cerrar").addEventListener("click",cerrar);
  var bc=document.getElementById("oc-gl-copiar");
  if(bc)bc.addEventListener("click",function(){
    var ok=function(){document.getElementById("oc-gl-msg").textContent="Copiada."};
    /* Sin clipboard API (http o navegador viejo) se cae al textarea de siempre
       antes que dejar al duenio sin poder copiarla. */
    if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(cod).then(ok,leg)}else{leg()}
    function leg(){try{var t=document.createElement("textarea");t.value=cod;t.style.position="fixed";t.style.opacity="0";
      document.body.appendChild(t);t.select();document.execCommand("copy");t.remove();ok()}
      catch(_){document.getElementById("oc-gl-msg").textContent="No se pudo copiar. Anótala a mano."}}
  });
});$("oc-pad").style.opacity="0.5";$("oc-pad").style.pointerEvents="none";$("oc-msg").style.color="var(--ink-soft,#5d5340)";$("oc-msg").textContent="Preparando tu acceso...";nuevoTeclado();listo.then(function(){$("oc-pad").style.opacity="";$("oc-pad").style.pointerEvents="";if($("oc-msg").textContent==="Preparando tu acceso..."){$("oc-msg").textContent=""}}).catch(function(){$("oc-pad").style.opacity="";$("oc-pad").style.pointerEvents="";if($("oc-msg").textContent==="Preparando tu acceso..."){$("oc-msg").textContent=""}});
...
function _ocMascaraLicencia(inp){
  ...
  /* 24 = AMG- + 4+1 + 4+1 + 4+1 + 5. El formato nuevo tiene CUATRO grupos y
     termina en el simbolo de verificacion de Crockford, que puede ser * ~ $ = o U.
     Con 18 la mascara truncaba el codigo nuevo a tres grupos y el dueno no podia
     teclear su propia licencia (encontrado trazando el flujo, 2026-08-14). */
  inp.setAttribute("maxlength", "24");
  // JFC 2026-07-30: "nunca pedi que se pre-escriba el AMG, quiero que la
  // gente lo escriba porque asi escriben mi marca" — antes esta funcion
  // BORRABA cualquier "AMG" que la persona escribiera y lo reponia a mano,
  // asi que technically nunca lo estaban escribiendo de verdad (aparecia
  // solo con el primer caracter que tocaran). Ahora es puramente mecanica:
  // solo agrega los 3 guiones en las posiciones fijas (3, 7, 11), nunca
  // toca ni fuerza el contenido — si la persona no escribe "AMG", el campo
  // simplemente no arranca con "AMG" (la validacion en otro lado exige el
  // formato completo antes de aceptar el codigo).
  function formatear(raw){
    var v = String(raw || "").toUpperCase();
    /* Los simbolos de verificacion de Crockford (* ~ $ = U) son parte del codigo
       y NO se pueden filtrar: si se caen, el codigo queda incompleto. */
    v = v.replace(/[^A-Z0-9*~$=]/g, "");     // fuera guiones y espacios
    v = v.slice(0, 20);                       // AMG + 16 + simbolo = 20
    var out = v.slice(0, 3);
    if (v.length > 3) out += "-" + v.slice(3, 7);
    if (v.length > 7) out += "-" + v.slice(7, 11);
    if (v.length > 11) out += "-" + v.slice(11, 15);
    if (v.length > 15) out += "-" + v.slice(15, 20);
    return out;
  }
  function alEscribir(){
    // JFC 2026-07-30 ("esa caja es DETESTABLE, al borrar la A se salta al
    // fondo y borra el ultimo caracter en vez del que estabas editando"):
    // antes SIEMPRE mandaba el cursor al final tras reformatear, sin
    // importar donde estaba editando. Eso hacia que borrar en medio del
    // texto (pegado desde apuntes, con guiones distintos, etc.) siguiera
    // borrando el ultimo caracter una y otra vez. Ahora se cuenta cuantos
    // caracteres alfanumericos habia ANTES del cursor en el valor viejo, y
    // se reubica el cursor despues de esa misma cantidad en el valor ya
    // formateado (los guiones no cuentan, son puestos por formatear()).
    var antes = inp.value;
    var posAntes = inp.selectionStart != null ? inp.selectionStart : antes.length;
    var alnumAntes = 0;
    for (var k = 0; k < posAntes && k < antes.length; k++){
      if (/[A-Za-z0-9]/.test(antes[k])) alnumAntes++;
    }
    var despues = formatear(antes);
    if (antes !== despues){
      inp.value = despues;
      var nuevaPos = despues.length, contados = 0;
      if (alnumAntes === 0) {
        nuevaPos = despues.indexOf("-") >= 0 ? despues.indexOf("-") + 1 : 0;
      } else {
        for (var j = 0; j < despues.length; j++){
          if (/[A-Za-z0-9]/.test(despues[j])){
            contados++;
            if (contados === alnumAntes) { nuevaPos = j + 1; break; }
          }
        }
      }
      try{ inp.setSelectionRange(nuevaPos, nuevaPos) }catch(_){}
    }
  }
  inp.addEventListener("input", alEscribir);
  inp.addEventListener("paste", function(){ setTimeout(alEscribir, 0) });
  // JFC 2026-07-30: "nunca pedi que se pre-escriba el AMG, quiero que la
  // gente lo escriba porque asi escriben mi marca" — SIN auto-sembrado en
  // focus. El campo se queda vacio hasta que la persona escribe; formatear()
  // sigue exigiendo que empiece con AMG (lo agrega solo si falta, ver abajo),
  // pero ya no lo inserta de prepo apenas hacen click.
}
  ...
  try{
    ...
    for (var i = 0; i < inputs.length; i++){
      (function(inp){
        ...
        // Icono real de ojo/ojo-tachado (JFC 2026-07-29, incidente en vivo con
        // un prospecto: el boton de texto "VER" no se reconocia como el
        // control para revisar la password, y sin poder verla, un typo en
        // "repite la password" parecia un error random del sistema). SVG
        // inline en vez de emoji: mismo estilo del resto de icoOC().
        var svgOjo = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>';
        var svgOjoTachado = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-7 0-11-8-11-8a18.5 18.5 0 0 1 5.06-5.94M9.9 4.24A10.94 10.94 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>';
        function pintar(){
          var oculto = inp.type === "password";
          b.innerHTML = oculto ? svgOjo : svgOjoTachado;
...
function abrirPoliticaPrivacidad(){
  ...
  cont.innerHTML=`<div class="caja" style="background:var(--blanco-calido,#fbf5e8);border:2px solid var(--brass,#9c7a35);border-radius:8px;padding:26px 22px;max-width:420px;width:100%;text-align:center;">
    ...
    <input id="oc-ue-codigo" type="text" placeholder="AMG-XXXX-XXXX-XXXX-XXXXX" style="width:100%;box-sizing:border-box;padding:11px 12px;border:2px solid var(--azul-medio,#2c4a68);border-radius:8px;font-size:16px;font-family:var(--font-mono,monospace);text-align:center;text-transform:uppercase;margin-bottom:10px;">
...
function pedirPasswordInicial(){
  ...
  cont.innerHTML='<div class="caja" style="'+_ocCaja()+'">'
    ...
    // Requisito real y COMPLETO, escrito como texto fijo — no solo en el
    // placeholder (JFC 2026-07-29: un placeholder desaparece apenas se
    // escribe, y bajo presion de una demo en vivo nadie se detiene a leerlo
    // antes de empezar a teclear). El unico requisito real es 6+ caracteres
    // — se dice explicitamente "nada mas" para que no se adivinen reglas
    // (mayuscula, simbolo) que esta app nunca exige.
    +'<p style="font-size:13px;color:var(--azul-medio,#2c4a68) !important;-webkit-text-fill-color:var(--azul-medio,#2c4a68) !important;margin:0 0 10px;">Requisito: al menos 6 caracteres — cualquier letra o numero sirve, no hace falta mayuscula ni simbolo.</p>'
...
function abrirResetConPassword(){
  ...
  cont.querySelector("#oc-rp-ok").addEventListener("click",async function(ev){
    ...
    var _okPin=await window.OCSecure.fijarOwnerPin(pin);
    if(!_okPin){msg.style.color="var(--rojo,#a3392a)";msg.textContent="No se pudo guardar el PIN nuevo (memoria del dispositivo llena). Tu PIN anterior sigue funcionando - libera espacio e intenta de nuevo.";return}
...
function abrirLiberarLicencia(){
  ...
  cont.innerHTML='<div class="caja" style="'+_ocCaja()+'">'
    ...
    +'<div style="background:#fff;border:2px dashed var(--brass,#9c7a35);border-radius:8px;padding:10px;margin-bottom:12px;"><span style="font-size:13px;color:var(--ink-soft,#5d5340) !important;-webkit-text-fill-color:var(--ink-soft,#5d5340) !important;">Tu licencia</span><br><code style="font-size:16px;font-weight:700;color:var(--ink,#211c14) !important;-webkit-text-fill-color:var(--ink,#211c14) !important;">'+(lic||"—")+'</code></div>'
