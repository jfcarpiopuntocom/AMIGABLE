...
(function (global) {
  ...
  function respuestaSinRed(url, err) {
    var cuerpo = JSON.stringify({
      error: "Sin conexión. No se pudo comunicar con el servidor.",
      offline: true,
    ...
    try {
      ...
      return {
        ok: false, status: 503, statusText: "Sin conexión", offline: true,
  ...
  global.fetch = function (entrada, opciones) {
    ...
    return anterior(entrada, opciones).catch(function (err) {
      ...
      try { console.warn("[red] sin conexión al pedir " + ultimo.url + " — la app sigue"); } catch (_) {}
