/*!
 * reconciliacion.js — Amigable-123 · MICELIO FASE B
 * ============================================================================
 * QUE ES
 * ----------------------------------------------------------------------------
...
(function (global) {
  ...
  // Antes de que los hechos puedan mandar de verdad (Fase C: reemplazar el
  // stock guardado por el reconstruido), hace falta ACUMULAR semanas de
  // comparaciones reales sin que nadie las mire — la prueba de que la
  // reconstruccion funciona antes de apostarle el negocio de alguien. Este
  // historial es esa prueba: nunca cambia una pantalla, nunca corrige nada.
  var HIST_KEY = "amg_reconciliacion_historial_v1";
  var HIST_TOPE = 120; // ~4 meses a 1/dia
  ...
  function montarPanel(mount) {
    ...
    mount.innerHTML =
      '<h3 class="seccion">Puntos de retorno</h3>'
      ...
      + 'La app guarda sola una copia completa de tu negocio cada día, aquí en tu dispositivo. '
      + 'Si algo se borra o se descuadra, puedes volver a como estaba. Ninguna copia reemplaza a otra: se suman.</p>'
      + '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;">'
      + '<button class="ir" id="oc-rec-crear">Guardar un punto de retorno ahora</button>'
      + '<button class="ir" id="oc-rec-revisar" style="background:transparent;color:#2E6278;border:2px solid #2E6278;">Revisar que todo cuadre</button>'
      + '</div>'
    ...
    function pintarLista() {
      listarArchivo().then(function (fichas) {
        ...
        if (!cont) return;
        if (!fichas.length) {
          cont.innerHTML = '<p style="font-size:15px;color:#2C3E50 !important;-webkit-text-fill-color:#2C3E50 !important;">Todavía no hay puntos de retorno guardados.</p>';
          return;
        }
        cont.innerHTML = '<table class="reporte"><thead><tr>'
          + '<th>Cuándo</th><th>Motivo</th><th>Productos</th><th>Ventas</th><th></th>'
          + '</tr></thead><tbody>'
          + fichas.map(function (f) {
            return '<tr><td>' + esc(fecha(f.ts)) + '</td><td>' + esc(f.motivo) + '</td>'
              ...
              + ' style="padding:6px 12px;min-height:44px;">Volver a este punto</button></td></tr>';
          }).join("")
          + '</tbody></table>';
      ...
      }).catch(function (e) { msg("No se pudo leer el archivo: " + e.message, "#E8365D"); });
    }

    // Restaurar pisa el estado actual, asi que se pregunta con todas las
    // letras. archivar() ya guarda el estado de hoy antes de pisarlo, y eso se
    // le DICE al dueno: saber que la accion es reversible es lo que permite
    // decidir sin miedo.
    function pedirRestaurar(id) {
      if (!global.confirm("Vas a volver tu negocio a como estaba en ese punto.\n\nLo de hoy NO se borra: se guarda primero como un punto de retorno nuevo, así que puedes deshacer esto.\n\n¿Continuar?")) return;
      msg("Restaurando…", "#2E6278");
      restaurar(id).then(function () {
        msg("Listo. Recarga la pantalla para ver los datos restaurados.", "#00C87A");
        pintarLista();
      }).catch(function (e) { msg("No se pudo restaurar: " + e.message, "#E8365D"); });
    }

    document.getElementById("oc-rec-crear").addEventListener("click", function () {
      msg("Guardando…", "#2E6278");
      archivar("a-mano").then(function () {
        msg("Punto de retorno guardado.", "#00C87A");
        pintarLista();
      }).catch(function (e) { msg("No se pudo guardar: " + e.message, "#E8365D"); });
    });

    document.getElementById("oc-rec-revisar").addEventListener("click", function () {
      msg("Revisando…", "#2E6278");
      comparar().then(function (inf) {
        if (!inf.discrepan.length) {
          msg("Todo cuadra. " + inf.coinciden + " producto(s) revisados con historial."
            + (inf.sinCobertura ? " (" + inf.sinCobertura + " todavía sin historial suficiente.)" : ""), "#00C87A");
          return;
        }
        // Se informa, NO se corrige. Una diferencia puede ser un error nuestro
        // o un faltante real en la percha, y solo el dueno sabe cual.
        var det = inf.discrepan.slice(0, 6).map(function (d) {
          return "• " + d.nombre + ": la app dice " + d.segunEstado + ", el historial dice " + d.segunHechos;
        }).join("\n");
        msg(inf.discrepan.length + " producto(s) no cuadran. Revísalos en la percha.", "#E8A020");
        global.alert("Diferencias encontradas:\n\n" + det
          + (inf.discrepan.length > 6 ? "\n\n…y " + (inf.discrepan.length - 6) + " más." : "")
          + "\n\nNo cambiamos nada por nuestra cuenta: puede ser un faltante real. Cuenta esos productos en la percha y ajústalos tú.");
      }).catch(function (e) { msg("No se pudo revisar: " + e.message, "#E8365D"); });
    });
