/*!
 * geo-ping.js — amigable-123 · Control de ubicación del equipo
 * ============================================================================
 * Portado de friendly-123 (2026-07-30), donde ya está construido y probado
 * (roadmap Fase 7). Única diferencia real: amigable-123 no tiene capa de
 * i18n (window.t/window.tf), así que aquí el texto va fijo en español en
 * vez de pasar por T()/TF() — mismo comportamiento, sin la capa que no existe.
 *
 * QUE ES ESTO
 * ----------------------------------------------------------------------------
...
(function (global) {
  ...
  // de una vez seguida, arrancarParaSesion() podia llamar esto dos veces
  // antes de que la primera terminara, mostrando 2 overlays apilados.
  // _avisoPendiente cachea la promesa en curso: una segunda llamada
  // mientras la primera sigue abierta espera la MISMA respuesta en vez de
  ...
  // Blindaje (misma logica que friendly-123): el panel debe MONTARSE una
  // sola vez pero su VISIBILIDAD se re-evalua en cada cambio de sesion — si
  // no, un dueño que cierra sesion y le pasa el mismo dispositivo a un
  // encargado dejaria el panel del equipo completo visible para ese
  // encargado, que es exactamente el tipo de fuga de datos entre roles que
  // este proyecto blinda en todos lados. Se re-chequea el rol, nunca se
  // asume que "montado" significa "debe seguir visible".
