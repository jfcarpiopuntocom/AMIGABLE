// sync-yjs.js — PLAN C del sync (CRDT/Yjs). PORTADO A AMIGABLE desde friendly-123
// (incluye handshake de catch-up + backoff de reconexion). ROOM_KEY
// amigable_sync_room, relay amigable-sync-relay, prefijos amig-yjs.
//
...
(function () {
  ...
  /* SYNC NUEVO ENCENDIDO POR DEFECTO (JFC 2026-09-15, homologado de friendly-123
     v267). Antes estaba detrás del flag apagado y por eso los BYTES de las fotos
     nunca cruzaban entre aparatos (solo este motor los mueve). Ahora corre para
     todos, EN PARALELO al sync viejo. Escotilla: si el toggle guarda "0"
     explícito, se apaga. Sin clave (null) = ON. Room-gated en arrancar(): costo
     cero para el que trabaja solo. */
  ...
  var ROOM_KEY = "amigable_sync_room";
  var SALT_FIJO = "amigable-sync-v1";
  var RELAY_URL = "wss://amigable-sync-relay.jfcarpio.workers.dev/sala/";
  ...
  async function arrancar() {
    ...
    try { API.idb = new window.IndexeddbPersistence("amig-yjs-" + API.roomId, API.doc); } catch (e) { log("idb:", e && e.message); }

    // Convergencia entre pestañas del MISMO origen: instantánea, sin red.
    try {
      API.bc = new BroadcastChannel("amig-yjs-" + API.roomId);
    ...
    try { API.fotosIdb = new window.IndexeddbPersistence("amig-yjs-fotos-" + API.roomId, API.fotosDoc); } catch (_) {}
    try {
      API.fotosBc = new BroadcastChannel("amig-yjs-fotos-" + API.roomId);
    ...
    try { API.opsIdb = new window.IndexeddbPersistence("amig-yjs-ops-" + API.roomId, API.opsDoc); } catch (_) {}
    try {
      API.opsBc = new BroadcastChannel("amig-yjs-ops-" + API.roomId);
