...
(function () {
  ...
  const claveVieja = (id) => "vp_foto_percha_" + id;
    ...
    if (!SOPORTADO) {
      ...
      try {
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (k && k.indexOf("vp_foto_percha_") === 0) out[k.slice("vp_foto_percha_".length)] = localStorage.getItem(k);
        }
      } catch (_) {}
      return out;
    }
  ...
  // formato viejo (localStorage, vp_foto_percha_*) a IndexedDB y las borra
  ...
  async function migrarSiHaceFalta() {
    if (!SOPORTADO) return;
    const FLAG = "amg_fotos_migradas_idb_v1";
    if (localStorage.getItem(FLAG)) return;
    try {
      const claves = [];
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && k.indexOf("vp_foto_percha_") === 0) claves.push(k);
      }
      for (const k of claves) {
        const id = k.slice("vp_foto_percha_".length);
