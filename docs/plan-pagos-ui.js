/*!
 * plan-pagos-ui.js — amigable-123 · Interfaz de cuotas y abonos
 * ============================================================================
...
(function (global) {
  ...
  function adornar(clienteId) {
    ...
    global.AMG.PlanPagos.estadoDelPlan(clienteId).then(function (e) {
      ...
      if (e.proximoVencimiento) {
        ...
        p.className = "pp-prox";
        p.textContent = "Próxima cuota: " + fmt(e.montoCuota) + " el " +
      ...
      /* Anular el acuerdo. Solo el dueno, igual que fiar: un empleado no
         ...
         deuda, son dos cosas distintas. */
      if (esDueno()) {
        var b = document.createElement("button");
        b.className = "pp-anular";
        b.type = "button";
        b.textContent = "Cambiar acuerdo";
  ...
  function modalFiar(clienteId, nombre) {
    ...
    m.innerHTML =
      '<div class="pp-caja">' +
        '<h3>Fiar a ' + esc(nombre) + '</h3>' +
        '<div class="pp-fg"><label for="pp-monto">Cuánto se le fia</label>' +
          '<input id="pp-monto" type="number" inputmode="decimal" min="0" step="0.01"></div>' +
        '<div class="pp-fg"><label for="pp-motivo">Motivo (opcional)</label>' +
          '<input id="pp-motivo" type="text" autocomplete="off"></div>' +
        '<label class="pp-check"><input type="checkbox" id="pp-conplan"> Acordar un plan de pagos</label>' +
        '<div id="pp-campos" style="display:none;margin-top:12px;">' +
          '<div class="pp-fg"><label for="pp-cuotas">En cuántas cuotas</label>' +
            '<input id="pp-cuotas" type="number" inputmode="numeric" min="1" step="1" value="3"></div>' +
          '<div class="pp-fg"><label for="pp-frec">Cada cuánto</label>' +
            '<select id="pp-frec"><option value="mensual">Cada mes</option>' +
            '<option value="quincenal">Cada quince días</option>' +
            '<option value="semanal">Cada semana</option></select></div>' +
          '<div class="pp-fg"><label for="pp-desde">Primera cuota el</label>' +
            '<input id="pp-desde" type="date"></div>' +
          '<div class="pp-fg"><label for="pp-gracia">Avisar a partir de cuántos días de atraso</label>' +
            '<input id="pp-gracia" type="number" inputmode="numeric" min="0" step="1" value="0"></div>' +
          '<p class="pp-resumen" id="pp-resumen"></p>' +
        '</div>' +
        '<button type="button" class="pp-btn" id="pp-ok">Registrar</button>' +
        '<button type="button" class="pp-btn gris" id="pp-cancel">Cancelar</button>' +
    ...
    function resumir() {
      ...
      if (!(total > 0)) { $("pp-resumen").textContent = "Escribe primero cuánto se le fia."; return; }
      var cuota = total / n;
      var cada = { mensual: "al mes", quincenal: "cada quince días", semanal: "a la semana" }[$("pp-frec").value];
      $("pp-resumen").textContent = n + " cuotas de " + fmt(cuota) + " " + cada + ".";
    }
  ...
  /* Anular el acuerdo de cuotas. No borra nada ni toca el saldo: emite un
     hecho plan_pago_anulado, y a partir de ahi el motor deja de ver un plan
     activo. Si manana se acuerda otro, se crea uno nuevo y el historial
     muestra los dos. */
  function modalAnular(clienteId) {
    ...
    m.innerHTML =
      '<div class="pp-caja">' +
        '<h3>Cambiar el acuerdo de cuotas</h3>' +
        '<p class="pp-resumen">La deuda NO se perdona: el saldo queda igual. Lo que se anula es el calendario acordado. Si quieres, despues puedes acordar cuotas nuevas desde Fiar.</p>' +
        '<div class="pp-fg"><label for="pp-an-motivo">Por qué se cambia (opcional)</label>' +
          '<input id="pp-an-motivo" type="text" autocomplete="off"></div>' +
        '<button type="button" class="pp-btn" id="pp-an-ok">Anular el acuerdo</button>' +
        '<button type="button" class="pp-btn gris" id="pp-an-cancel">Dejarlo como está</button>' +
    ...
    m.querySelector("#pp-an-ok").addEventListener("click", function (ev) {
      ...
      global.AMG.PlanPagos.anularPlan(clienteId, m.querySelector("#pp-an-motivo").value || "")
        ...
        .catch(function (e) {
          ...
          msg.style.color = "#B0183E";
          msg.textContent = (e && e.message) || "No se pudo anular.";
        });
    });
  }
  ...
  function refrescarHoy() {
    ...
    clientesConEstado().then(function (todos) {
      ...
      caja.innerHTML =
        ...
        '<p class="s">Suman ' + fmt(total) + '. Son ' + (n === 1 ? "un cliente" : n + " clientes") +
          ' con plan de pagos acordado.</p>' +
  ...
  function montar() {
    ...
    if (cli && !document.getElementById("pp-filtro") && esDueno()) {
      ...
      f.id = "pp-filtro";
      f.innerHTML =
        '<button type="button" data-f="todos" class="on">Todos</button>' +
        '<button type="button" data-f="conplan">Con plan</button>' +
        '<button type="button" data-f="atrasados">Atrasados</button>';
